...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSsecurityPSfuzzingPSop_fuzzingPSmatmul_fuzzDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSsecurityPSfuzzingPSop_fuzzingPSmatmul_fuzzDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSsecurityPSfuzzingPSop_fuzzingPSmatmul_fuzzDTcc() {
      // Check if tracing is enabled
...
class FuzzMatMul : public FuzzSession {
  void BuildGraph(const Scope& scope) final {
   ...
   MHTracer_DTPStensorflowPSsecurityPSfuzzingPSop_fuzzingPSmatmul_fuzzDTcc mht_0(mht_0_v, 196, "", "./tensorflow/security/fuzzing/op_fuzzing/matmul_fuzz.cc", "BuildGraph");
  ...
  void FuzzImpl(const uint8_t* data, size_t size) final {
   ...
   MHTracer_DTPStensorflowPSsecurityPSfuzzingPSop_fuzzingPSmatmul_fuzzDTcc mht_1(mht_1_v, 206, "", "./tensorflow/security/fuzzing/op_fuzzing/matmul_fuzz.cc", "FuzzImpl");

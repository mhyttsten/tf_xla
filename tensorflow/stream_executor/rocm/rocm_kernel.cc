...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPSrocmPSrocm_kernelDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPSrocmPSrocm_kernelDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPSrocmPSrocm_kernelDTcc() {
      // Check if tracing is enabled
...
hipFuncCache_t GpuKernel::GetGpuCacheConfig() const {
   ...
   MHTracer_DTPStensorflowPSstream_executorPSrocmPSrocm_kernelDTcc mht_0(mht_0_v, 190, "", "./tensorflow/stream_executor/rocm/rocm_kernel.cc", "GpuKernel::GetGpuCacheConfig");

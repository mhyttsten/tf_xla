...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPSgpuPSgpu_asm_optsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPSgpuPSgpu_asm_optsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPSgpuPSgpu_asm_optsDTh() {
      // Check if tracing is enabled
...
struct GpuAsmOpts {
  ...
  PtxOptionsTuple ToTuple() {
   ...
   MHTracer_DTPStensorflowPSstream_executorPSgpuPSgpu_asm_optsDTh mht_0(mht_0_v, 216, "", "./tensorflow/stream_executor/gpu/gpu_asm_opts.h", "ToTuple");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh() {
      // Check if tracing is enabled
...
class TpuCoreLocationExternal {
 public:
  TpuCoreLocationExternal() : core_location_(nullptr) {
   ...
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh mht_0(mht_0_v, 204, "", "./tensorflow/stream_executor/tpu/tpu_topology.h", "TpuCoreLocationExternal");
}
  explicit TpuCoreLocationExternal(SE_TpuTopology_Core* core_location)
      : core_location_(core_location) {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh mht_1(mht_1_v, 209, "", "./tensorflow/stream_executor/tpu/tpu_topology.h", "TpuCoreLocationExternal");
}
  ...
  SE_TpuTopology_Core* impl() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh mht_2(mht_2_v, 218, "", "./tensorflow/stream_executor/tpu/tpu_topology.h", "impl");
 return core_location_; }
...
class TpuHostLocationExternal {
 public:
  explicit TpuHostLocationExternal(SE_TpuTopology_Host* host_location)
   ...
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh mht_3(mht_3_v, 230, "", "./tensorflow/stream_executor/tpu/tpu_topology.h", "TpuHostLocationExternal");
}
  int32 Id() const;
  std::vector<TpuCoreLocationExternal> Cores(TpuCoreTypeEnum core_type) const;

  SE_TpuTopology_Host* impl() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh mht_4(mht_4_v, 237, "", "./tensorflow/stream_executor/tpu/tpu_topology.h", "impl");
 return host_location_; }
...
class TpuTopologyExternal {
 public:
  explicit TpuTopologyExternal(SE_TpuTopology* topology)
   ...
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_topologyDTh mht_5(mht_5_v, 255, "", "./tensorflow/stream_executor/tpu/tpu_topology.h", "TpuTopologyExternal");
}

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_platform_interfaceDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_platform_interfaceDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_platform_interfaceDTh() {
      // Check if tracing is enabled
...
class TpuPlatformInterface : public stream_executor::Platform {
 public:
  ...
  Status Reset(absl::string_view reason) {
   std::vector<std::string> mht_0_v;
   mht_0_v.push_back("reason: \"" + std::string(reason.data(), reason.size()) + "\"");
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_platform_interfaceDTh mht_0(mht_0_v, 218, "", "./tensorflow/stream_executor/tpu/tpu_platform_interface.h", "Reset");
 return Reset(false, reason); }

  Status Reset() {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_platform_interfaceDTh mht_1(mht_1_v, 223, "", "./tensorflow/stream_executor/tpu/tpu_platform_interface.h", "Reset");
 return Reset(false, {}); }
  ...
  TpuTopologyExternal topology() {
   ...
   MHTracer_DTPStensorflowPSstream_executorPStpuPStpu_platform_interfaceDTh mht_2(mht_2_v, 238, "", "./tensorflow/stream_executor/tpu/tpu_platform_interface.h", "topology");

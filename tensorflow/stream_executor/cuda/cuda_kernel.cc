...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPScudaPScuda_kernelDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPScudaPScuda_kernelDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPScudaPScuda_kernelDTcc() {
      // Check if tracing is enabled
...
CUfunc_cache GpuKernel::GetGpuCacheConfig() const {
   ...
   MHTracer_DTPStensorflowPSstream_executorPScudaPScuda_kernelDTcc mht_0(mht_0_v, 190, "", "./tensorflow/stream_executor/cuda/cuda_kernel.cc", "GpuKernel::GetGpuCacheConfig");

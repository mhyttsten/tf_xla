...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPSplatformPSdefaultPSinitializeDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPSplatformPSdefaultPSinitializeDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPSplatformPSdefaultPSinitializeDTh() {
      // Check if tracing is enabled
...
class Initializer {
 public:
  ...
  explicit Initializer(InitializerFunc func) {
   std::vector<std::string> mht_0_v;
   MHTracer_DTPStensorflowPSstream_executorPSplatformPSdefaultPSinitializeDTh mht_0(mht_0_v, 198, "", "./tensorflow/stream_executor/platform/default/initialize.h", "Initializer");
 func(); }

  struct Dependency {
    Dependency(const char *n, Initializer *i) : name(n), initializer(i) {
   std::vector<std::string> mht_1_v;
   mht_1_v.push_back("n: \"" + (n == nullptr ? std::string("nullptr") : std::string((char*)n)) + "\"");
   MHTracer_DTPStensorflowPSstream_executorPSplatformPSdefaultPSinitializeDTh mht_1(mht_1_v, 205, "", "./tensorflow/stream_executor/platform/default/initialize.h", "Dependency");
}

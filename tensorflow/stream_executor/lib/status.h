...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPSlibPSstatusDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlibPSstatusDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPSlibPSstatusDTh() {
      // Check if tracing is enabled
...
inline Status InvalidArgumentError(absl::string_view message) {
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlibPSstatusDTh mht_0(mht_0_v, 209, "", "./tensorflow/stream_executor/lib/status.h", "InvalidArgumentError");
...
inline Status InternalError(absl::string_view message) {
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlibPSstatusDTh mht_1(mht_1_v, 216, "", "./tensorflow/stream_executor/lib/status.h", "InternalError");

  return Status(error::INTERNAL, message);
}
inline Status FailedPreconditionError(absl::string_view message) {
   std::vector<std::string> mht_2_v;
   mht_2_v.push_back("message: \"" + std::string(message.data(), message.size()) + "\"");
   MHTracer_DTPStensorflowPSstream_executorPSlibPSstatusDTh mht_2(mht_2_v, 223, "", "./tensorflow/stream_executor/lib/status.h", "FailedPreconditionError");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh() {
      // Check if tracing is enabled
...
struct Dim3D {
  ...
  Dim3D(uint64_t x, uint64 y, uint64 z) : x(x), y(y), z(z) {
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh mht_0(mht_0_v, 218, "", "./tensorflow/stream_executor/launch_dim.h", "Dim3D");
}
};
...
struct ThreadDim : public Dim3D {
  explicit ThreadDim(uint64_t x = 1, uint64 y = 1, uint64 z = 1)
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh mht_1(mht_1_v, 228, "", "./tensorflow/stream_executor/launch_dim.h", "ThreadDim");
}

  // Returns a string representation of the thread dimensionality.
  std::string ToString() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh mht_2(mht_2_v, 234, "", "./tensorflow/stream_executor/launch_dim.h", "ToString");
...
struct BlockDim : public Dim3D {
  explicit BlockDim(uint64_t x = 1, uint64 y = 1, uint64 z = 1)
   ...
   MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh mht_3(mht_3_v, 246, "", "./tensorflow/stream_executor/launch_dim.h", "BlockDim");
}

  // Returns a string representation of the block dimensionality.
  std::string ToString() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSstream_executorPSlaunch_dimDTh mht_4(mht_4_v, 252, "", "./tensorflow/stream_executor/launch_dim.h", "ToString");

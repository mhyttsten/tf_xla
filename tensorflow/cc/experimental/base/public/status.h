...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh() {
      // Check if tracing is enabled
...
class Status {
 public:
  ...
  Status() : status_(TF_NewStatus()) {
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_0(mht_0_v, 207, "", "./tensorflow/cc/experimental/base/public/status.h", "Status");
}
 ...
 private:
  ...
  explicit Status(TF_Status* status) : status_(status) {
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_1(mht_1_v, 236, "", "./tensorflow/cc/experimental/base/public/status.h", "Status");
}
  ...
  TF_Status* GetTFStatus() const {
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_2(mht_2_v, 247, "", "./tensorflow/cc/experimental/base/public/status.h", "GetTFStatus");
 return status_.get(); }
...
inline TF_Code Status::code() const {
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_3(mht_3_v, 258, "", "./tensorflow/cc/experimental/base/public/status.h", "Status::code");
 return TF_GetCode(status_.get()); }

inline std::string Status::message() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_4(mht_4_v, 263, "", "./tensorflow/cc/experimental/base/public/status.h", "Status::message");
...
inline bool Status::ok() const {
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_5(mht_5_v, 270, "", "./tensorflow/cc/experimental/base/public/status.h", "Status::ok");
 return code() == TF_OK; }

inline void Status::SetStatus(TF_Code code, const std::string& msg) {
   std::vector<std::string> mht_6_v;
   mht_6_v.push_back("msg: \"" + msg + "\"");
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePSpublicPSstatusDTh mht_6(mht_6_v, 276, "", "./tensorflow/cc/experimental/base/public/status.h", "Status::SetStatus");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensor_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensor_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensor_testDTcc() {
      // Check if tracing is enabled
...
TYPED_TEST(ConstructScalarTensorTest, ValidTensorAttributesAfterConstruction) {
  ...
  Tensor tensor = Tensor::FromBuffer(/*dtype=*/dtype, /*shape=*/{},
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensor_testDTcc mht_0(mht_0_v, 219, "", "./tensorflow/cc/experimental/base/tests/tensor_test.cc", "lambda");
}, &status);
...
TEST(CPPTensorAPI, ConstructTensorFromBuffer) {
  ...
  std::vector<int32_t> data_vector({12, 14, 20, 18, 39, 42, 100});
  {
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensor_testDTcc mht_1(mht_1_v, 319, "", "./tensorflow/cc/experimental/base/tests/tensor_test.cc", "lambda");

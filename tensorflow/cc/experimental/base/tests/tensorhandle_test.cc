...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensorhandle_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensorhandle_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensorhandle_testDTcc() {
      // Check if tracing is enabled
...
TYPED_TEST(ConstructScalarTensorHandleTest,
  ...
  Tensor original_tensor =
   ...
   MHTracer_DTPStensorflowPSccPSexperimentalPSbasePStestsPStensorhandle_testDTcc mht_0(mht_0_v, 234, "", "./tensorflow/cc/experimental/base/tests/tensorhandle_test.cc", "lambda");
}, &status);
  ASSERT_TRUE(status.ok()) << status.message();

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSgradientsPSfunctional_gradDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSfunctional_gradDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSgradientsPSfunctional_gradDTcc() {
      // Check if tracing is enabled
...
Status PartitionedCallGrad(const Scope& scope, const Operation& op,
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSfunctional_gradDTcc mht_0(mht_0_v, 198, "", "./tensorflow/cc/gradients/functional_grad.cc", "PartitionedCallGrad");

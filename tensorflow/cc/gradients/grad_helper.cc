...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSgradientsPSgrad_helperDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSgrad_helperDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSgradientsPSgrad_helperDTcc() {
      // Check if tracing is enabled
...
Output ReducedShapeHelper(const Scope& scope, const Output& input_shape,
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSgrad_helperDTcc mht_0(mht_0_v, 202, "", "./tensorflow/cc/gradients/grad_helper.cc", "ReducedShapeHelper");

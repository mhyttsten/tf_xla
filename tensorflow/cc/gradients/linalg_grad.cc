...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc() {
      // Check if tracing is enabled
...
Output Slice1dHelper(const Scope& scope, Output tensor, int start,
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc mht_0(mht_0_v, 271, "", "./tensorflow/cc/gradients/linalg_grad.cc", "Slice1dHelper");
...
Output EinsumGradReducedHelper(const Scope& scope, const Output& output_grad,
   ...
   mht_1_v.push_back("output_subs: \"" + std::string(output_subs.data(), output_subs.size()) + "\"");
   mht_1_v.push_back("input_subs: \"" + std::string(input_subs.data(), input_subs.size()) + "\"");
   MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc mht_1(mht_1_v, 362, "", "./tensorflow/cc/gradients/linalg_grad.cc", "EinsumGradReducedHelper");
...
Output EinsumGradWrt(const Scope& scope, Output output_grad,
   ...
   mht_2_v.push_back("input_subs: \"" + std::string(input_subs.data(), input_subs.size()) + "\"");
   mht_2_v.push_back("other_subs: \"" + std::string(other_subs.data(), other_subs.size()) + "\"");
   mht_2_v.push_back("output_subs: \"" + std::string(output_subs.data(), output_subs.size()) + "\"");
   MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc mht_2(mht_2_v, 464, "", "./tensorflow/cc/gradients/linalg_grad.cc", "EinsumGradWrt");
...
Status EinsumGrad(const Scope& scope, const Operation& op,
   ...
   MHTracer_DTPStensorflowPSccPSgradientsPSlinalg_gradDTcc mht_3(mht_3_v, 544, "", "./tensorflow/cc/gradients/linalg_grad.cc", "EinsumGrad");

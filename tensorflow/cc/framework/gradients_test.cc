...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc() {
      // Check if tracing is enabled
...
class GradientsTest : public ::testing::Test {
 protected:
  GradientsTest()
   ...
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc mht_0(mht_0_v, 217, "", "./tensorflow/cc/framework/gradients_test.cc", "GradientsTest");
}

  void CompareTestAndExpectedGraphs() {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc mht_1(mht_1_v, 222, "", "./tensorflow/cc/framework/gradients_test.cc", "CompareTestAndExpectedGraphs");
...
class StopGradientSingleOutputMultiEdgeTest : public ::testing::Test {
 protected:
  StopGradientSingleOutputMultiEdgeTest() : scope_(Scope::NewRootScope()) {
   ...
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc mht_2(mht_2_v, 722, "", "./tensorflow/cc/framework/gradients_test.cc", "StopGradientSingleOutputMultiEdgeTest");
}

  void CheckGrad(const std::vector<bool>& stop_outputs,
                 const Tensor& expected_grad) {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc mht_3(mht_3_v, 728, "", "./tensorflow/cc/framework/gradients_test.cc", "CheckGrad");
...
class StopGradientMultiOutputTest : public ::testing::Test {
 protected:
  StopGradientMultiOutputTest() : scope_(Scope::NewRootScope()) {
   ...
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc mht_4(mht_4_v, 810, "", "./tensorflow/cc/framework/gradients_test.cc", "StopGradientMultiOutputTest");
}

  void CheckGrad(const std::vector<bool>& stop_outputs,
                 const Tensor& expected_grad) {
   std::vector<std::string> mht_5_v;
   MHTracer_DTPStensorflowPSccPSframeworkPSgradients_testDTcc mht_5(mht_5_v, 816, "", "./tensorflow/cc/framework/gradients_test.cc", "CheckGrad");

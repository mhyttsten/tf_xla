...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSccPSframeworkPSscope_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSccPSframeworkPSscope_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSccPSframeworkPSscope_testDTcc() {
      // Check if tracing is enabled
...
string LastOp(const Scope& scope) {
   std::vector<std::string> mht_0_v;
   MHTracer_DTPStensorflowPSccPSframeworkPSscope_testDTcc mht_0(mht_0_v, 249, "", "./tensorflow/cc/framework/scope_test.cc", "LastOp");

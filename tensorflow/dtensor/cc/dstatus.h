...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSdtensorPSccPSdstatusDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSdtensorPSccPSdstatusDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSdtensorPSccPSdstatusDTh() {
      // Check if tracing is enabled
...
inline Status WithContext(const Status& ds, absl::string_view file,
   ...
   mht_0_v.push_back("file: \"" + std::string(file.data(), file.size()) + "\"");
   MHTracer_DTPStensorflowPSdtensorPSccPSdstatusDTh mht_0(mht_0_v, 207, "", "./tensorflow/dtensor/cc/dstatus.h", "WithContext");

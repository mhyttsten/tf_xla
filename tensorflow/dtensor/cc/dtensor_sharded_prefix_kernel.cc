...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSdtensorPSccPSdtensor_sharded_prefix_kernelDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSdtensorPSccPSdtensor_sharded_prefix_kernelDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSdtensorPSccPSdtensor_sharded_prefix_kernelDTcc() {
      // Check if tracing is enabled
...
class DTensorShardedPrefixOpKernel : public OpKernel {
 public:
  explicit DTensorShardedPrefixOpKernel(OpKernelConstruction* ctx)
   ...
   MHTracer_DTPStensorflowPSdtensorPSccPSdtensor_sharded_prefix_kernelDTcc mht_0(mht_0_v, 202, "", "./tensorflow/dtensor/cc/dtensor_sharded_prefix_kernel.cc", "DTensorShardedPrefixOpKernel");
}

 private:
  void Compute(OpKernelContext* ctx) override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSdtensorPSccPSdtensor_sharded_prefix_kernelDTcc mht_1(mht_1_v, 208, "", "./tensorflow/dtensor/cc/dtensor_sharded_prefix_kernel.cc", "Compute");

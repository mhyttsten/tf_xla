...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSdtensorPSmlirPSop_utilsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSdtensorPSmlirPSop_utilsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSdtensorPSmlirPSop_utilsDTh() {
      // Check if tracing is enabled
...
inline std::string OpName(mlir::Operation* op) {
   ...
   MHTracer_DTPStensorflowPSdtensorPSmlirPSop_utilsDTh mht_0(mht_0_v, 201, "", "./tensorflow/dtensor/mlir/op_utils.h", "OpName");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSlibPSioPSzlib_outputbufferDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSlibPSioPSzlib_outputbufferDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSlibPSioPSzlib_outputbufferDTh() {
      // Check if tracing is enabled
...
class ZlibOutputBuffer : public WritableFile {
 ...
 private:
  ...
  static bool IsSyncOrFullFlush(uint8 flush_mode) {
   ...
   MHTracer_DTPStensorflowPScorePSlibPSioPSzlib_outputbufferDTh mht_0(mht_0_v, 317, "", "./tensorflow/core/lib/io/zlib_outputbuffer.h", "IsSyncOrFullFlush");

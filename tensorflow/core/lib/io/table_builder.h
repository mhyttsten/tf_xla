...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSlibPSioPStable_builderDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSlibPSioPStable_builderDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSlibPSioPStable_builderDTh() {
      // Check if tracing is enabled
...
class TableBuilder {
 ...
 private:
  bool ok() const {
   ...
   MHTracer_DTPStensorflowPScorePSlibPSioPStable_builderDTh mht_0(mht_0_v, 252, "", "./tensorflow/core/lib/io/table_builder.h", "ok");
 return status().ok(); }

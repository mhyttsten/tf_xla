...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSlibPSioPStableDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSlibPSioPStableDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSlibPSioPStableDTh() {
      // Check if tracing is enabled
...
class Table {
 ...
 private:
  ...
  explicit Table(Rep* rep) {
   ...
   MHTracer_DTPStensorflowPScorePSlibPSioPStableDTh mht_0(mht_0_v, 237, "", "./tensorflow/core/lib/io/table.h", "Table");
 rep_ = rep; }

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSlibPSpngPSpng_ioDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSlibPSpngPSpng_ioDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSlibPSpngPSpng_ioDTh() {
      // Check if tracing is enabled
...
struct DecodeContext {
  ...
  DecodeContext() : png_ptr(nullptr), info_ptr(nullptr) {
   ...
   MHTracer_DTPStensorflowPScorePSlibPSpngPSpng_ioDTh mht_0(mht_0_v, 228, "", "./tensorflow/core/lib/png/png_io.h", "DecodeContext");
}
};

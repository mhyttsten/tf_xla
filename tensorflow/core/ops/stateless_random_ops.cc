...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSopsPSstateless_random_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSopsPSstateless_random_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSopsPSstateless_random_opsDTcc() {
      // Check if tracing is enabled
...
static Status StatelessShape(InferenceContext* c) {
   ...
   MHTracer_DTPStensorflowPScorePSopsPSstateless_random_opsDTcc mht_0(mht_0_v, 194, "", "./tensorflow/core/ops/stateless_random_ops.cc", "StatelessShape");

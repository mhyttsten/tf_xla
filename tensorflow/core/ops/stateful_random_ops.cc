...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSopsPSstateful_random_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSopsPSstateful_random_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSopsPSstateful_random_opsDTcc() {
      // Check if tracing is enabled
...
Status StatefulRandomShape(shape_inference::InferenceContext* c) {
   ...
   MHTracer_DTPStensorflowPScorePSopsPSstateful_random_opsDTcc mht_0(mht_0_v, 192, "", "./tensorflow/core/ops/stateful_random_ops.cc", "StatefulRandomShape");

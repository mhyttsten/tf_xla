...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSopsPSctc_ops_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSopsPSctc_ops_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSopsPSctc_ops_testDTcc() {
      // Check if tracing is enabled
...
TEST(CtcOpsTest, CTCBeamSearchDecoder_ShapeFn) {
  ...
  auto set_top_paths = [&op](int top_paths) {
   ...
   MHTracer_DTPStensorflowPScorePSopsPSctc_ops_testDTcc mht_0(mht_0_v, 237, "", "./tensorflow/core/ops/ctc_ops_test.cc", "lambda");

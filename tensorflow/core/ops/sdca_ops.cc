...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSopsPSsdca_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSopsPSsdca_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSopsPSsdca_opsDTcc() {
      // Check if tracing is enabled
...
static Status ApplySdcaOptimizerShapeFn(InferenceContext* c) {
   ...
   MHTracer_DTPStensorflowPScorePSopsPSsdca_opsDTcc mht_0(mht_0_v, 195, "", "./tensorflow/core/ops/sdca_ops.cc", "ApplySdcaOptimizerShapeFn");

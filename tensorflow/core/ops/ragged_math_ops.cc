...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSopsPSragged_math_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSopsPSragged_math_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSopsPSragged_math_opsDTcc() {
      // Check if tracing is enabled
...
Status RaggedRangeShapeFn(InferenceContext* c) {
   ...
   MHTracer_DTPStensorflowPScorePSopsPSragged_math_opsDTcc mht_0(mht_0_v, 214, "", "./tensorflow/core/ops/ragged_math_ops.cc", "RaggedRangeShapeFn");

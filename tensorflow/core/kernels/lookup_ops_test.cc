...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSlookup_ops_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSlookup_ops_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSlookup_ops_testDTcc() {
      // Check if tracing is enabled
...
class MockHashTable : public lookup::HashTable<K, V> {
 public:
  MockHashTable(OpKernelContext* ctx, OpKernel* kernel)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSlookup_ops_testDTcc mht_0(mht_0_v, 217, "", "./tensorflow/core/kernels/lookup_ops_test.cc", "MockHashTable");

    alive = true;
  }
  ~MockHashTable() override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSlookup_ops_testDTcc mht_1(mht_1_v, 223, "", "./tensorflow/core/kernels/lookup_ops_test.cc", "~MockHashTable");
 alive = false; }
};

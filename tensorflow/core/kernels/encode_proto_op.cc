...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc() {
      // Check if tracing is enabled
...
static void WriteStringAdapter(int field_number, const tstring& value,
   ...
   mht_0_v.push_back("value: \"" + (std::string)value + "\"");
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc mht_0(mht_0_v, 470, "", "./tensorflow/core/kernels/encode_proto_op.cc", "WriteStringAdapter");
...
static void WriteBytesAdapter(int field_number, const tstring& value,
   ...
   mht_1_v.push_back("value: \"" + (std::string)value + "\"");
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc mht_1(mht_1_v, 484, "", "./tensorflow/core/kernels/encode_proto_op.cc", "WriteBytesAdapter");
...
Status WriteGroup(const FieldDescriptor& field_desc, const Tensor& input,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc mht_2(mht_2_v, 500, "", "./tensorflow/core/kernels/encode_proto_op.cc", "WriteGroup");
...
Status WriteField(const FieldDescriptor& field_desc, const Tensor& input,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc mht_3(mht_3_v, 522, "", "./tensorflow/core/kernels/encode_proto_op.cc", "WriteField");
...
class EncodeProtoOp : public OpKernel {
 public:
  explicit EncodeProtoOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc mht_4(mht_4_v, 662, "", "./tensorflow/core/kernels/encode_proto_op.cc", "EncodeProtoOp");
  ...
  void Compute(OpKernelContext* ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSencode_proto_opDTcc mht_5(mht_5_v, 710, "", "./tensorflow/core/kernels/encode_proto_op.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSlinalgPScholesky_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSlinalgPScholesky_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSlinalgPScholesky_opDTcc() {
      // Check if tracing is enabled
...
class CholeskyOp : public LinearAlgebraOp<Scalar> {
 public:
  ...
  explicit CholeskyOp(OpKernelConstruction* context) : Base(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSlinalgPScholesky_opDTcc mht_0(mht_0_v, 205, "", "./tensorflow/core/kernels/linalg/cholesky_op.cc", "CholeskyOp");
}

  void ComputeMatrix(OpKernelContext* context, const ConstMatrixMaps& inputs,
                     MatrixMaps* outputs) final {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSlinalgPScholesky_opDTcc mht_1(mht_1_v, 211, "", "./tensorflow/core/kernels/linalg/cholesky_op.cc", "ComputeMatrix");

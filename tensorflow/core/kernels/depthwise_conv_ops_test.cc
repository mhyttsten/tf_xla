...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_ops_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_ops_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_ops_testDTcc() {
      // Check if tracing is enabled
...
class DepthwiseConvOpTest : public OpsTestBase {
 protected:
  ...
  void Run(Device device) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_ops_testDTcc mht_0(mht_0_v, 210, "", "./tensorflow/core/kernels/depthwise_conv_ops_test.cc", "Run");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSmklPSmkl_relu_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSmklPSmkl_relu_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSmklPSmkl_relu_op_testDTcc() {
      // Check if tracing is enabled
...
static Graph* Activation(const string& op_name, const string& kind,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSmklPSmkl_relu_op_testDTcc mht_0(mht_0_v, 218, "", "./tensorflow/core/kernels/mkl/mkl_relu_op_test.cc", "Activation");

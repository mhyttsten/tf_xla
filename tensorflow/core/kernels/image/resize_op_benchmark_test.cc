...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSimagePSresize_op_benchmark_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSresize_op_benchmark_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSimagePSresize_op_benchmark_testDTcc() {
      // Check if tracing is enabled
...
static Graph* Resize(const char* algorithm, int batches, int width,
   ...
   mht_0_v.push_back("algorithm: \"" + (algorithm == nullptr ? std::string("nullptr") : std::string((char*)algorithm)) + "\"");
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSresize_op_benchmark_testDTcc mht_0(mht_0_v, 195, "", "./tensorflow/core/kernels/image/resize_op_benchmark_test.cc", "Resize");

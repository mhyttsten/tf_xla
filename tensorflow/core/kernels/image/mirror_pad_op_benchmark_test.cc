...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSimagePSmirror_pad_op_benchmark_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSmirror_pad_op_benchmark_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSimagePSmirror_pad_op_benchmark_testDTcc() {
      // Check if tracing is enabled
...
static Graph* MirrorPad(int batches, int height, int width, int depth, int pad,
   ...
   mht_0_v.push_back("mode: \"" + (mode == nullptr ? std::string("nullptr") : std::string((char*)mode)) + "\"");
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSmirror_pad_op_benchmark_testDTcc mht_0(mht_0_v, 195, "", "./tensorflow/core/kernels/image/mirror_pad_op_benchmark_test.cc", "MirrorPad");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSimagePSattention_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSattention_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSimagePSattention_opsDTcc() {
      // Check if tracing is enabled
...
class ExtractGlimpseOp : public OpKernel {
 public:
  explicit ExtractGlimpseOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSattention_opsDTcc mht_0(mht_0_v, 203, "", "./tensorflow/core/kernels/image/attention_ops.cc", "ExtractGlimpseOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSattention_opsDTcc mht_1(mht_1_v, 240, "", "./tensorflow/core/kernels/image/attention_ops.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSimagePSextract_volume_patches_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSextract_volume_patches_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSimagePSextract_volume_patches_opDTcc() {
      // Check if tracing is enabled
...
static inline void ParseAttributeVec5(OpKernelConstruction* context,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSextract_volume_patches_opDTcc mht_0(mht_0_v, 220, "", "./tensorflow/core/kernels/image/extract_volume_patches_op.cc", "ParseAttributeVec5");
...
class ExtractVolumePatchesOp : public UnaryOp<T> {
 public:
  explicit ExtractVolumePatchesOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSextract_volume_patches_opDTcc mht_1(mht_1_v, 236, "", "./tensorflow/core/kernels/image/extract_volume_patches_op.cc", "ExtractVolumePatchesOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSimagePSextract_volume_patches_opDTcc mht_2(mht_2_v, 246, "", "./tensorflow/core/kernels/image/extract_volume_patches_op.cc", "Compute");

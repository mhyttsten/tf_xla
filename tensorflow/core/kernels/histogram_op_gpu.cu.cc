...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPShistogram_op_gpuDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPShistogram_op_gpuDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPShistogram_op_gpuDTcuDTcc() {
      // Check if tracing is enabled
...
struct HistogramFixedWidthFunctor<GPUDevice, T, Tout> {
  static Status Compute(OpKernelContext* context,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPShistogram_op_gpuDTcuDTcc mht_0(mht_0_v, 212, "", "./tensorflow/core/kernels/histogram_op_gpu.cu.cc", "Compute");

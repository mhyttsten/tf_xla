...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSstrided_slice_opDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstrided_slice_opDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSstrided_slice_opDTh() {
      // Check if tracing is enabled
...
struct InitOutput {
  static void run(const Device& d, typename TTypes<T, NDIMS>::Tensor output) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstrided_slice_opDTh mht_0(mht_0_v, 218, "", "./tensorflow/core/kernels/strided_slice_op.h", "run");
...
struct InitOutput<ResourceHandle, NDIMS, Device> {
  static void run(const Device& d,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstrided_slice_opDTh mht_1(mht_1_v, 229, "", "./tensorflow/core/kernels/strided_slice_op.h", "run");
...
struct InitOutput<tstring, NDIMS, Device> {
  static void run(const Device& d,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstrided_slice_opDTh mht_2(mht_2_v, 240, "", "./tensorflow/core/kernels/strided_slice_op.h", "run");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc() {
      // Check if tracing is enabled
...
inline void CopyToBuffer(const T& value, uint8* output) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc mht_0(mht_0_v, 199, "", "./tensorflow/core/kernels/fingerprint_op.cc", "CopyToBuffer");
...
void FarmhashFingerprint64(TTypes<uint8, 2>::ConstTensor input,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc mht_1(mht_1_v, 216, "", "./tensorflow/core/kernels/fingerprint_op.cc", "FarmhashFingerprint64");
...
void FarmhashFingerprint64(TTypes<tstring>::ConstFlat input,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc mht_2(mht_2_v, 231, "", "./tensorflow/core/kernels/fingerprint_op.cc", "FarmhashFingerprint64");
...
class FingerprintOp : public OpKernel {
 public:
  explicit FingerprintOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc mht_3(mht_3_v, 246, "", "./tensorflow/core/kernels/fingerprint_op.cc", "FingerprintOp");
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfingerprint_opDTcc mht_4(mht_4_v, 257, "", "./tensorflow/core/kernels/fingerprint_op.cc", "Compute");

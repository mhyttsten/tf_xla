...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc() {
      // Check if tracing is enabled
...
ReduceDetails SparseTensorReduceHelper(const SparseTensor &sp,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_0(mht_0_v, 227, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "SparseTensorReduceHelper");
...
Status ValidateInputs(const Tensor *shape_t, const Tensor *reduction_axes_t) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_1(mht_1_v, 280, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "ValidateInputs");
...
struct SumOp {
  ...
  static void Run(OpKernelContext *ctx, typename TTypes<T>::Scalar &s, const typename TTypes<T>::UnalignedVec &v) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_2(mht_2_v, 312, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "Run");

      s.device(ctx->eigen_cpu_device()) = v.sum();
  }
  static StringPiece Name() {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_3(mht_3_v, 318, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "Name");
...
struct MaxOp {
  ...
  static void Run(OpKernelContext *ctx, typename TTypes<T>::Scalar &s, const typename TTypes<T>::UnalignedVec &v) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_4(mht_4_v, 328, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "Run");

      s.device(ctx->eigen_cpu_device()) = v.maximum();
  }
  static StringPiece Name() {
   std::vector<std::string> mht_5_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_5(mht_5_v, 334, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "Name");
...
class SparseReduceOp : public OpKernel {
 public:
  explicit SparseReduceOp(OpKernelConstruction *ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_6(mht_6_v, 345, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "SparseReduceOp");
  ...
  void Compute(OpKernelContext *ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_7(mht_7_v, 352, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "Compute");
...
class SparseReduceSparseOp : public OpKernel {
 public:
  explicit SparseReduceSparseOp(OpKernelConstruction *ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_8(mht_8_v, 463, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "SparseReduceSparseOp");
  ...
  void Compute(OpKernelContext *ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_reduce_opDTcc mht_9(mht_9_v, 470, "", "./tensorflow/core/kernels/sparse_reduce_op.cc", "Compute");

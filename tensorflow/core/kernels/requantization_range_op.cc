...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSrequantization_range_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantization_range_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSrequantization_range_opDTcc() {
      // Check if tracing is enabled
...
void CalculateUsedRange(const Tensor& input, qint32* used_min_quantized,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantization_range_opDTcc mht_0(mht_0_v, 204, "", "./tensorflow/core/kernels/requantization_range_op.cc", "CalculateUsedRange");
...
class RequantizationRangeOp : public OpKernel {
 public:
  explicit RequantizationRangeOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantization_range_opDTcc mht_1(mht_1_v, 217, "", "./tensorflow/core/kernels/requantization_range_op.cc", "RequantizationRangeOp");
}

  void Compute(OpKernelContext* ctx) override {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantization_range_opDTcc mht_2(mht_2_v, 222, "", "./tensorflow/core/kernels/requantization_range_op.cc", "Compute");

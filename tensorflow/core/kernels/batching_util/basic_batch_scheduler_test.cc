...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc() {
      // Check if tracing is enabled
...
class FakeTask : public BatchTask {
 public:
  explicit FakeTask(size_t size) : size_(size) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc mht_0(mht_0_v, 201, "", "./tensorflow/core/kernels/batching_util/basic_batch_scheduler_test.cc", "FakeTask");
}
  ...
  size_t size() const override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc mht_1(mht_1_v, 208, "", "./tensorflow/core/kernels/batching_util/basic_batch_scheduler_test.cc", "size");
 return size_; }
...
Status ScheduleTask(size_t task_size, BatchScheduler<FakeTask>* scheduler) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc mht_2(mht_2_v, 221, "", "./tensorflow/core/kernels/batching_util/basic_batch_scheduler_test.cc", "ScheduleTask");
...
TEST(BasicBatchSchedulerTest, Basic) {
  ...
  auto callback = [&callback_called](std::unique_ptr<Batch<FakeTask>> batch) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbatching_utilPSbasic_batch_scheduler_testDTcc mht_3(mht_3_v, 238, "", "./tensorflow/core/kernels/batching_util/basic_batch_scheduler_test.cc", "lambda");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSgather_functor_batchedDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSgather_functor_batchedDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSgather_functor_batchedDTh() {
      // Check if tracing is enabled
...
SliceIndex HandleCopiesBatched(OpKernelContext* ctx,
  ...
  auto work = [&](int64_t start, int64_t end) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSgather_functor_batchedDTh mht_0(mht_0_v, 230, "", "./tensorflow/core/kernels/gather_functor_batched.h", "lambda");

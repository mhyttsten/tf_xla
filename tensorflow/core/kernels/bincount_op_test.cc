...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSbincount_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbincount_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSbincount_op_testDTcc() {
      // Check if tracing is enabled
...
static Graph* Bincount(int arr_size, int nbins) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbincount_op_testDTcc mht_0(mht_0_v, 196, "", "./tensorflow/core/kernels/bincount_op_test.cc", "Bincount");

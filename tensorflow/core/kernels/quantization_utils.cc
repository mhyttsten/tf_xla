...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSquantization_utilsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantization_utilsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSquantization_utilsDTcc() {
      // Check if tracing is enabled
...
void GetOutputMinAndMaxForQuantizedAdd(float input_min, float input_max,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantization_utilsDTcc mht_0(mht_0_v, 192, "", "./tensorflow/core/kernels/quantization_utils.cc", "GetOutputMinAndMaxForQuantizedAdd");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_using_gemmDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_using_gemmDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_using_gemmDTcc() {
      // Check if tracing is enabled
...
class Im2ColConvFunctor {
 public:
  void operator()(OpKernelContext* context, const T1* input_data,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_using_gemmDTcc mht_0(mht_0_v, 481, "", "./tensorflow/core/kernels/conv_ops_using_gemm.cc", "lambda");
...
class Conv2DUsingGemmOp : public BinaryOp<T> {
 public:
  explicit Conv2DUsingGemmOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_using_gemmDTcc mht_1(mht_1_v, 606, "", "./tensorflow/core/kernels/conv_ops_using_gemm.cc", "Conv2DUsingGemmOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_using_gemmDTcc mht_2(mht_2_v, 630, "", "./tensorflow/core/kernels/conv_ops_using_gemm.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsparse_add_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_add_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsparse_add_op_testDTcc() {
      // Check if tracing is enabled
...
class SparseAddOpTest : public OpsTestBase {
 protected:
  ...
  void MakeOp() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_add_op_testDTcc mht_0(mht_0_v, 203, "", "./tensorflow/core/kernels/sparse_add_op_test.cc", "MakeOp");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSbetainc_opDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbetainc_opDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSbetainc_opDTh() {
      // Check if tracing is enabled
...
struct Betainc {
  ...
  void BCast(const Device& d, typename TTypes<T, NDIM>::ConstTensor a,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbetainc_opDTh mht_0(mht_0_v, 211, "", "./tensorflow/core/kernels/betainc_op.h", "BCast");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSquantized_conv_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_conv_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSquantized_conv_opsDTcc() {
      // Check if tracing is enabled
...
class Im2ColConvFunctor {
 public:
  void operator()(OpKernelContext* context, const T1* input_data,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_conv_opsDTcc mht_0(mht_0_v, 447, "", "./tensorflow/core/kernels/quantized_conv_ops.cc", "lambda");
...
class QuantizedConv2DOp : public OpKernel {
 public:
  explicit QuantizedConv2DOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_conv_opsDTcc mht_1(mht_1_v, 633, "", "./tensorflow/core/kernels/quantized_conv_ops.cc", "QuantizedConv2DOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_conv_opsDTcc mht_2(mht_2_v, 665, "", "./tensorflow/core/kernels/quantized_conv_ops.cc", "Compute");

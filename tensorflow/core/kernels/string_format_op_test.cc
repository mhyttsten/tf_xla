...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSstring_format_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_format_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSstring_format_op_testDTcc() {
      // Check if tracing is enabled
...
class StringFormatGraphTest : public OpsTestBase {
 protected:
  Status Init(int num_inputs, DataType input_type,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_format_op_testDTcc mht_0(mht_0_v, 203, "", "./tensorflow/core/kernels/string_format_op_test.cc", "Init");

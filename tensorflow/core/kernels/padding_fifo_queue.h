...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queueDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queueDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queueDTh() {
      // Check if tracing is enabled
...
class PaddingFIFOQueue : public FIFOQueue {
 ...
 private:
  ~PaddingFIFOQueue() override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queueDTh mht_0(mht_0_v, 244, "", "./tensorflow/core/kernels/padding_fifo_queue.h", "~PaddingFIFOQueue");
}

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc() {
      // Check if tracing is enabled
...
static Status ReadEntireFile(Env* env, const string& filename, T* contents) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_0(mht_0_v, 208, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "ReadEntireFile");
...
class WholeFileReader : public ReaderBase {
 public:
  WholeFileReader(Env* env, const string& node_name)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_1(mht_1_v, 225, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "WholeFileReader");
}

  Status ReadLocked(tstring* key, tstring* value, bool* produced,
                    bool* at_end) override {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_2(mht_2_v, 231, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "ReadLocked");
  ...
  Status SerializeStateLocked(tstring* state) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_3(mht_3_v, 244, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "SerializeStateLocked");
  ...
  Status RestoreStateLocked(const tstring& state) override {
   ...
   mht_4_v.push_back("state: \"" + (std::string)state + "\"");
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_4(mht_4_v, 255, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "RestoreStateLocked");
...
class WholeFileReaderOp : public ReaderOpKernel {
 public:
  explicit WholeFileReaderOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_5(mht_5_v, 275, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "WholeFileReaderOp");
...
class ReadFileOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_6(mht_6_v, 293, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "Compute");
...
class WriteFileOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSwhole_file_read_opsDTcc mht_7(mht_7_v, 318, "", "./tensorflow/core/kernels/whole_file_read_ops.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSquantized_mul_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_mul_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSquantized_mul_opDTcc() {
      // Check if tracing is enabled
...
class QuantizedMulOp : public OpKernel {
 public:
  explicit QuantizedMulOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_mul_opDTcc mht_0(mht_0_v, 452, "", "./tensorflow/core/kernels/quantized_mul_op.cc", "QuantizedMulOp");
}

  void Compute(OpKernelContext* context) override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_mul_opDTcc mht_1(mht_1_v, 457, "", "./tensorflow/core/kernels/quantized_mul_op.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc() {
      // Check if tracing is enabled
...
struct DepthwiseConv2DKernel {
  static void Run(const DepthwiseArgs& args,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc mht_0(mht_0_v, 260, "", "./tensorflow/core/kernels/depthwise_conv_op.cc", "Run");
...
struct LaunchDepthwiseConvOp<CPUDevice, T> {
  ...
  void operator()(OpKernelContext* ctx, const DepthwiseArgs& args,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc mht_1(mht_1_v, 367, "", "./tensorflow/core/kernels/depthwise_conv_op.cc", "lambda");
...
bool DisableDepthwiseConvDeterminismExceptions() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc mht_2(mht_2_v, 443, "", "./tensorflow/core/kernels/depthwise_conv_op.cc", "DisableDepthwiseConvDeterminismExceptions");
...
class DepthwiseConv2dNativeOp : public BinaryOp<T> {
 public:
  explicit DepthwiseConv2dNativeOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc mht_3(mht_3_v, 463, "", "./tensorflow/core/kernels/depthwise_conv_op.cc", "DepthwiseConv2dNativeOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc mht_4(mht_4_v, 519, "", "./tensorflow/core/kernels/depthwise_conv_op.cc", "Compute");
...
class DepthwiseConv2dGroupedConvOp
 ...
 public:
  DepthwiseConv2dGroupedConvOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdepthwise_conv_opDTcc mht_5(mht_5_v, 717, "", "./tensorflow/core/kernels/depthwise_conv_op.cc", "DepthwiseConv2dGroupedConvOp");

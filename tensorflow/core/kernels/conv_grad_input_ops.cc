...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_input_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_input_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_input_opsDTcc() {
      // Check if tracing is enabled
...
struct ConvBackwardDataAutotuneGroup {
  static string name() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_input_opsDTcc mht_0(mht_0_v, 212, "", "./tensorflow/core/kernels/conv_grad_input_ops.cc", "name");
 return "ConvBwdData"; }
};

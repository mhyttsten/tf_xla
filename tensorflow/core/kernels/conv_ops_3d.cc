...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc() {
      // Check if tracing is enabled
...
struct LaunchConvOp<CPUDevice, T> {
  static void launch(OpKernelContext* context, bool cudnn_use_autotune,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc mht_0(mht_0_v, 233, "", "./tensorflow/core/kernels/conv_ops_3d.cc", "launch");
...
class Conv3DOp : public BinaryOp<T> {
 public:
  explicit Conv3DOp(OpKernelConstruction* context) : BinaryOp<T>(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc mht_1(mht_1_v, 261, "", "./tensorflow/core/kernels/conv_ops_3d.cc", "Conv3DOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc mht_2(mht_2_v, 305, "", "./tensorflow/core/kernels/conv_ops_3d.cc", "Compute");
...
struct Conv3dAutotuneGroup {
  static string name() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc mht_3(mht_3_v, 396, "", "./tensorflow/core/kernels/conv_ops_3d.cc", "name");
 return "Conv3d"; }
};
...
struct LaunchConvOp<GPUDevice, T> {
  static void launch(OpKernelContext* ctx, bool cudnn_use_autotune,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_ops_3dDTcc mht_4(mht_4_v, 413, "", "./tensorflow/core/kernels/conv_ops_3d.cc", "launch");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSmultinomial_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSmultinomial_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSmultinomial_op_testDTcc() {
      // Check if tracing is enabled
...
static Graph* Multinomial(int batch_size, int num_classes, int num_samples) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSmultinomial_op_testDTcc mht_0(mht_0_v, 196, "", "./tensorflow/core/kernels/multinomial_op_test.cc", "Multinomial");

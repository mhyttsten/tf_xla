...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSconcat_lib_gpuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconcat_lib_gpuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSconcat_lib_gpuDTcc() {
      // Check if tracing is enabled
...
void ConcatGPU(
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconcat_lib_gpuDTcc mht_0(mht_0_v, 244, "", "./tensorflow/core/kernels/concat_lib_gpu.cc", "ConcatGPU");

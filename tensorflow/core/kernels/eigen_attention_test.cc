...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSeigen_attention_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSeigen_attention_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSeigen_attention_testDTcc() {
      // Check if tracing is enabled
...
void EigenApprox(float a, float b) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSeigen_attention_testDTcc mht_0(mht_0_v, 191, "", "./tensorflow/core/kernels/eigen_attention_test.cc", "EigenApprox");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc() {
      // Check if tracing is enabled
...
class QuantizedReluOp : public OpKernel {
 public:
  explicit QuantizedReluOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc mht_0(mht_0_v, 200, "", "./tensorflow/core/kernels/quantized_activation_ops.cc", "QuantizedReluOp");
}

  void Compute(OpKernelContext* context) override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc mht_1(mht_1_v, 205, "", "./tensorflow/core/kernels/quantized_activation_ops.cc", "Compute");
...
class QuantizedRelu6Op : public OpKernel {
 public:
  explicit QuantizedRelu6Op(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc mht_2(mht_2_v, 239, "", "./tensorflow/core/kernels/quantized_activation_ops.cc", "QuantizedRelu6Op");
}

  void Compute(OpKernelContext* context) override {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_activation_opsDTcc mht_3(mht_3_v, 244, "", "./tensorflow/core/kernels/quantized_activation_ops.cc", "Compute");

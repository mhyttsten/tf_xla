...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh() {
      // Check if tracing is enabled
...
class ReshapeOp : public OpKernel {
 public:
  explicit ReshapeOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh mht_0(mht_0_v, 204, "", "./tensorflow/core/kernels/reshape_op.h", "ReshapeOp");
}

  void Compute(OpKernelContext* context) override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh mht_1(mht_1_v, 209, "", "./tensorflow/core/kernels/reshape_op.h", "Compute");
  ...
  bool IsExpensive() override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh mht_2(mht_2_v, 284, "", "./tensorflow/core/kernels/reshape_op.h", "IsExpensive");
 return false; }
 ...
 private:
  ...
  Status ValidateSizes(const Tensor& sizes, int64_t* product,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_opDTh mht_3(mht_3_v, 293, "", "./tensorflow/core/kernels/reshape_op.h", "ValidateSizes");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSstackDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstackDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSstackDTh() {
      // Check if tracing is enabled
...
class TemplatedStackPushOp : public StackPushOp {
 public:
  TemplatedStackPushOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstackDTh mht_0(mht_0_v, 226, "", "./tensorflow/core/kernels/stack.h", "TemplatedStackPushOp");
}
};

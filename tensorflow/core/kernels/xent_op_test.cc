...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSxent_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSxent_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSxent_op_testDTcc() {
      // Check if tracing is enabled
...
static Graph* Xent(int batch_size, int num_classes, DataType type) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSxent_op_testDTcc mht_0(mht_0_v, 194, "", "./tensorflow/core/kernels/xent_op_test.cc", "Xent");

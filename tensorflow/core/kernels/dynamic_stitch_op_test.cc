...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSdynamic_stitch_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdynamic_stitch_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSdynamic_stitch_op_testDTcc() {
      // Check if tracing is enabled
...
class DynamicStitchOpTest : public OpsTestBase {
 protected:
  void MakeOp(int n, DataType dt) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdynamic_stitch_op_testDTcc mht_0(mht_0_v, 208, "", "./tensorflow/core/kernels/dynamic_stitch_op_test.cc", "MakeOp");

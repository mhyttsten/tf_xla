...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsparsePSconj_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparsePSconj_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsparsePSconj_opDTcc() {
      // Check if tracing is enabled
...
class CSRSparseMatrixConjFunctor {
 public:
  explicit CSRSparseMatrixConjFunctor(OpKernelContext* ctx) : ctx_(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparsePSconj_opDTcc mht_0(mht_0_v, 217, "", "./tensorflow/core/kernels/sparse/conj_op.cc", "CSRSparseMatrixConjFunctor");
}

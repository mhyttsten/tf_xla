...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSrandom_poisson_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrandom_poisson_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSrandom_poisson_opDTcc() {
      // Check if tracing is enabled
...
struct PoissonFunctor<CPUDevice, T, U> {
  void operator()(OpKernelContext* ctx, const CPUDevice& d, const T* rate_flat,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrandom_poisson_opDTcc mht_0(mht_0_v, 269, "", "./tensorflow/core/kernels/random_poisson_op.cc", "lambda");
...
class RandomPoissonOp : public OpKernel {
 public:
  explicit RandomPoissonOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrandom_poisson_opDTcc mht_1(mht_1_v, 460, "", "./tensorflow/core/kernels/random_poisson_op.cc", "RandomPoissonOp");
  ...
  void Compute(OpKernelContext* ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrandom_poisson_opDTcc mht_2(mht_2_v, 467, "", "./tensorflow/core/kernels/random_poisson_op.cc", "Compute");

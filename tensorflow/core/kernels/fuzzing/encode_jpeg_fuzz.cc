...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSfuzzingPSencode_jpeg_fuzzDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfuzzingPSencode_jpeg_fuzzDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSfuzzingPSencode_jpeg_fuzzDTcc() {
      // Check if tracing is enabled
...
class FuzzEncodeJpeg : public FuzzSession {
  ...
  void FuzzImpl(const uint8_t* data, size_t size) final {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfuzzingPSencode_jpeg_fuzzDTcc mht_0(mht_0_v, 194, "", "./tensorflow/core/kernels/fuzzing/encode_jpeg_fuzz.cc", "FuzzImpl");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh() {
      // Check if tracing is enabled
...
struct ConvBackpropDimensions {
  ...
  int64_t input_size(int dim) const {
   std::vector<std::string> mht_0_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh mht_0(mht_0_v, 225, "", "./tensorflow/core/kernels/conv_grad_shape_utils.h", "input_size");
 return spatial_dims[dim].input_size; }
  int64_t filter_size(int dim) const {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh mht_1(mht_1_v, 229, "", "./tensorflow/core/kernels/conv_grad_shape_utils.h", "filter_size");
 return spatial_dims[dim].filter_size; }
  int64_t output_size(int dim) const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh mht_2(mht_2_v, 233, "", "./tensorflow/core/kernels/conv_grad_shape_utils.h", "output_size");
 return spatial_dims[dim].output_size; }
  int64_t stride(int dim) const {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh mht_3(mht_3_v, 237, "", "./tensorflow/core/kernels/conv_grad_shape_utils.h", "stride");
 return spatial_dims[dim].stride; }
  int64_t dilation(int dim) const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_shape_utilsDTh mht_4(mht_4_v, 241, "", "./tensorflow/core/kernels/conv_grad_shape_utils.h", "dilation");

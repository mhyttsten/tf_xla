...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_opDTcc() {
      // Check if tracing is enabled
...
class InTopK : public OpKernel {
 public:
  explicit InTopK(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_opDTcc mht_0(mht_0_v, 203, "", "./tensorflow/core/kernels/in_topk_op.cc", "InTopK");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_opDTcc mht_1(mht_1_v, 212, "", "./tensorflow/core/kernels/in_topk_op.cc", "Compute");

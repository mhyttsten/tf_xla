...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSrestore_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrestore_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSrestore_op_testDTcc() {
      // Check if tracing is enabled
...
class RestoreOpTest : public OpsTestBase {
 protected:
  ...
  void MakeRestoreOp(DataType dt) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrestore_op_testDTcc mht_0(mht_0_v, 214, "", "./tensorflow/core/kernels/restore_op_test.cc", "MakeRestoreOp");
...
Tensor MakeInput(const TensorShape& shape,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrestore_op_testDTcc mht_1(mht_1_v, 230, "", "./tensorflow/core/kernels/restore_op_test.cc", "MakeInput");
...
class RestoreSliceOpTest : public OpsTestBase {
 protected:
  void MakeRestoreSliceOp(DataType dt) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrestore_op_testDTcc mht_2(mht_2_v, 539, "", "./tensorflow/core/kernels/restore_op_test.cc", "MakeRestoreSliceOp");

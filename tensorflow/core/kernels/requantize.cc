...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSrequantizeDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantizeDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSrequantizeDTcc() {
      // Check if tracing is enabled
...
class RequantizeOp : public OpKernel {
 public:
  explicit RequantizeOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantizeDTcc mht_0(mht_0_v, 206, "", "./tensorflow/core/kernels/requantize.cc", "RequantizeOp");
}

  void Compute(OpKernelContext* ctx) override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantizeDTcc mht_1(mht_1_v, 211, "", "./tensorflow/core/kernels/requantize.cc", "Compute");

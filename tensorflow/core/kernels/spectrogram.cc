...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc() {
      // Check if tracing is enabled
...
void GetPeriodicHann(int window_length, std::vector<double>* window) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc mht_0(mht_0_v, 198, "", "./tensorflow/core/kernels/spectrogram.cc", "GetPeriodicHann");
...
bool Spectrogram::Initialize(int window_length, int step_length) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc mht_1(mht_1_v, 211, "", "./tensorflow/core/kernels/spectrogram.cc", "Spectrogram::Initialize");
...
bool Spectrogram::Initialize(const std::vector<double>& window,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc mht_2(mht_2_v, 221, "", "./tensorflow/core/kernels/spectrogram.cc", "Spectrogram::Initialize");
...
bool Spectrogram::Reset() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc mht_3(mht_3_v, 258, "", "./tensorflow/core/kernels/spectrogram.cc", "Spectrogram::Reset");
...
bool Spectrogram::GetNextWindowOfSamples(const std::vector<InputSample>& input,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc mht_4(mht_4_v, 366, "", "./tensorflow/core/kernels/spectrogram.cc", "Spectrogram::GetNextWindowOfSamples");
...
void Spectrogram::ProcessCoreFFT() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSspectrogramDTcc mht_5(mht_5_v, 393, "", "./tensorflow/core/kernels/spectrogram.cc", "Spectrogram::ProcessCoreFFT");

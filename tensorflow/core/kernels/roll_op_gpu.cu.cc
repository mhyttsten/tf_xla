...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSroll_op_gpuDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSroll_op_gpuDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSroll_op_gpuDTcuDTcc() {
      // Check if tracing is enabled
...
__global__ void RollKernel(const int32 nthreads, const int32 num_dims,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSroll_op_gpuDTcuDTcc mht_0(mht_0_v, 206, "", "./tensorflow/core/kernels/roll_op_gpu.cu.cc", "RollKernel");

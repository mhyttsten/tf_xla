...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSrequantize_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantize_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSrequantize_op_testDTcc() {
      // Check if tracing is enabled
...
class RequantizeTest : public OpsTestBase {
 protected:
  void ConfigureRequantize() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrequantize_op_testDTcc mht_0(mht_0_v, 202, "", "./tensorflow/core/kernels/requantize_op_test.cc", "ConfigureRequantize");

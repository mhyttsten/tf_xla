...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSfused_batch_norm_opDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfused_batch_norm_opDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSfused_batch_norm_opDTcuDTcc() {
      // Check if tracing is enabled
...
struct FusedBatchNormInferenceKernel {
  ...
  __device__ static void run(int32 count, int32 channels_size,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfused_batch_norm_opDTcuDTcc mht_0(mht_0_v, 302, "", "./tensorflow/core/kernels/fused_batch_norm_op.cu.cc", "run");
...
struct FusedBatchNormInferenceKernel<Eigen::half, float, tensor_format,
  ...
  __device__ static void run(int32 count, int32 channels_size,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSfused_batch_norm_opDTcuDTcc mht_1(mht_1_v, 367, "", "./tensorflow/core/kernels/fused_batch_norm_op.cu.cc", "run");

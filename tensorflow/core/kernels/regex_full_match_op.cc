...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc() {
      // Check if tracing is enabled
...
class RegexFullMatchOp : public OpKernel {
 public:
  explicit RegexFullMatchOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc mht_0(mht_0_v, 200, "", "./tensorflow/core/kernels/regex_full_match_op.cc", "RegexFullMatchOp");
}

  ~RegexFullMatchOp() override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc mht_1(mht_1_v, 205, "", "./tensorflow/core/kernels/regex_full_match_op.cc", "~RegexFullMatchOp");
}

  void Compute(OpKernelContext* ctx) override {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc mht_2(mht_2_v, 210, "", "./tensorflow/core/kernels/regex_full_match_op.cc", "Compute");
...
class StaticRegexFullMatchOp : public OpKernel {
 public:
  explicit StaticRegexFullMatchOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc mht_3(mht_3_v, 268, "", "./tensorflow/core/kernels/regex_full_match_op.cc", "StaticRegexFullMatchOp");
  ...
  void Compute(OpKernelContext* ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSregex_full_match_opDTcc mht_4(mht_4_v, 280, "", "./tensorflow/core/kernels/regex_full_match_op.cc", "Compute");

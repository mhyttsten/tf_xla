...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSqueue_opsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSqueue_opsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSqueue_opsDTcc() {
      // Check if tracing is enabled
...
class FakeQueueOp : public OpKernel {
 public:
  explicit FakeQueueOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSqueue_opsDTcc mht_0(mht_0_v, 248, "", "./tensorflow/core/kernels/queue_ops.cc", "FakeQueueOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSqueue_opsDTcc mht_1(mht_1_v, 256, "", "./tensorflow/core/kernels/queue_ops.cc", "Compute");

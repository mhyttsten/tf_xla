...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSreshape_util_gpuDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_util_gpuDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSreshape_util_gpuDTcuDTcc() {
      // Check if tracing is enabled
...
__global__ void ReshapeSparseTensorKernel(
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSreshape_util_gpuDTcuDTcc mht_0(mht_0_v, 204, "", "./tensorflow/core/kernels/reshape_util_gpu.cu.cc", "ReshapeSparseTensorKernel");

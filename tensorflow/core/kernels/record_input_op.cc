...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSrecord_input_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrecord_input_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSrecord_input_opDTcc() {
      // Check if tracing is enabled
...
class RecordInputOp : public OpKernel {
 public:
  explicit RecordInputOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrecord_input_opDTcc mht_0(mht_0_v, 197, "", "./tensorflow/core/kernels/record_input_op.cc", "RecordInputOp");
...
#undef GETATTR
  ...
  void Compute(OpKernelContext* ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSrecord_input_opDTcc mht_1(mht_1_v, 228, "", "./tensorflow/core/kernels/record_input_op.cc", "Compute");

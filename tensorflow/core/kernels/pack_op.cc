...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSpack_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpack_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSpack_opDTcc() {
      // Check if tracing is enabled
...
class PackOp : public OpKernel {
 public:
  ...
  explicit PackOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpack_opDTcc mht_0(mht_0_v, 214, "", "./tensorflow/core/kernels/pack_op.cc", "PackOp");
  ...
  void Compute(OpKernelContext* c) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpack_opDTcc mht_1(mht_1_v, 221, "", "./tensorflow/core/kernels/pack_op.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queue_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queue_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queue_opDTcc() {
      // Check if tracing is enabled
...
class PaddingFIFOQueueOp : public TypedQueueOp {
 public:
  explicit PaddingFIFOQueueOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpadding_fifo_queue_opDTcc mht_0(mht_0_v, 215, "", "./tensorflow/core/kernels/padding_fifo_queue_op.cc", "PaddingFIFOQueueOp");

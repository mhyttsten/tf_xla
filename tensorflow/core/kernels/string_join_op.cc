...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSstring_join_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_join_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSstring_join_opDTcc() {
      // Check if tracing is enabled
...
class StringJoinOp : public OpKernel {
 public:
  ...
  explicit StringJoinOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_join_opDTcc mht_0(mht_0_v, 202, "", "./tensorflow/core/kernels/string_join_op.cc", "StringJoinOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_join_opDTcc mht_1(mht_1_v, 209, "", "./tensorflow/core/kernels/string_join_op.cc", "Compute");

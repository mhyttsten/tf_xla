...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPStensor_to_hash_bucket_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStensor_to_hash_bucket_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPStensor_to_hash_bucket_opDTcc() {
      // Check if tracing is enabled
...
class TensorToHashBucketOp : public OpKernel {
 public:
  explicit TensorToHashBucketOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStensor_to_hash_bucket_opDTcc mht_0(mht_0_v, 198, "", "./tensorflow/core/kernels/tensor_to_hash_bucket_op.cc", "TensorToHashBucketOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStensor_to_hash_bucket_opDTcc mht_1(mht_1_v, 216, "", "./tensorflow/core/kernels/tensor_to_hash_bucket_op.cc", "Compute");

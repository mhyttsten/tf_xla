...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsequence_ops_gpuDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsequence_ops_gpuDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsequence_ops_gpuDTcuDTcc() {
      // Check if tracing is enabled
...
__global__ void RangeKernel(int64_t size, T start, T delta,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsequence_ops_gpuDTcuDTcc mht_0(mht_0_v, 202, "", "./tensorflow/core/kernels/sequence_ops_gpu.cu.cc", "RangeKernel");

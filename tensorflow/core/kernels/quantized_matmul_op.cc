...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSquantized_matmul_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_matmul_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSquantized_matmul_opDTcc() {
      // Check if tracing is enabled
...
class QuantizedMatMulOp : public OpKernel {
 public:
  explicit QuantizedMatMulOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_matmul_opDTcc mht_0(mht_0_v, 239, "", "./tensorflow/core/kernels/quantized_matmul_op.cc", "QuantizedMatMulOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSquantized_matmul_opDTcc mht_1(mht_1_v, 247, "", "./tensorflow/core/kernels/quantized_matmul_op.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSguarantee_const_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSguarantee_const_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSguarantee_const_op_testDTcc() {
      // Check if tracing is enabled
...
class GuaranteeConstOpTest : public OpsTestBase {
 protected:
  Status Init(DataType input_type) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSguarantee_const_op_testDTcc mht_0(mht_0_v, 202, "", "./tensorflow/core/kernels/guarantee_const_op_test.cc", "Init");

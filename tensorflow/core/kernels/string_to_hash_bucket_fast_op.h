...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSstring_to_hash_bucket_fast_opDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_to_hash_bucket_fast_opDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSstring_to_hash_bucket_fast_opDTh() {
      // Check if tracing is enabled
...
class StringToHashBucketOp : public OpKernel {
 public:
  explicit StringToHashBucketOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_to_hash_bucket_fast_opDTh mht_0(mht_0_v, 201, "", "./tensorflow/core/kernels/string_to_hash_bucket_fast_op.h", "StringToHashBucketOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSstring_to_hash_bucket_fast_opDTh mht_1(mht_1_v, 208, "", "./tensorflow/core/kernels/string_to_hash_bucket_fast_op.h", "Compute");

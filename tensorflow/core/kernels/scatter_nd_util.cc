...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSscatter_nd_utilDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSscatter_nd_utilDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSscatter_nd_utilDTcc() {
      // Check if tracing is enabled
...
Status ValidateScatterNdUpdateShape(const TensorShape& params_shape,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSscatter_nd_utilDTcc mht_0(mht_0_v, 193, "", "./tensorflow/core/kernels/scatter_nd_util.cc", "ValidateScatterNdUpdateShape");
  ...
  auto shape_err_prefix = [&]() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSscatter_nd_utilDTcc mht_1(mht_1_v, 204, "", "./tensorflow/core/kernels/scatter_nd_util.cc", "lambda");
  ...
  auto shape_err_suffix = [&]() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSscatter_nd_utilDTcc mht_2(mht_2_v, 214, "", "./tensorflow/core/kernels/scatter_nd_util.cc", "lambda");

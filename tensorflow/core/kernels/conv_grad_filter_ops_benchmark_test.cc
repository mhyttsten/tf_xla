...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_filter_ops_benchmark_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_filter_ops_benchmark_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_filter_ops_benchmark_testDTcc() {
      // Check if tracing is enabled
...
static Tensor MakeRandomTensor(const TensorShape& shape) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_filter_ops_benchmark_testDTcc mht_0(mht_0_v, 202, "", "./tensorflow/core/kernels/conv_grad_filter_ops_benchmark_test.cc", "MakeRandomTensor");
...
static Graph* Conv2DBackpropFilter(int batch, int height, int width,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSconv_grad_filter_ops_benchmark_testDTcc mht_1(mht_1_v, 215, "", "./tensorflow/core/kernels/conv_grad_filter_ops_benchmark_test.cc", "Conv2DBackpropFilter");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSidentity_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSidentity_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSidentity_op_testDTcc() {
      // Check if tracing is enabled
...
class IdentityOpTest : public OpsTestBase {
 protected:
  Status Init(DataType input_type) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSidentity_op_testDTcc mht_0(mht_0_v, 201, "", "./tensorflow/core/kernels/identity_op_test.cc", "Init");

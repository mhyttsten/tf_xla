...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSlossDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSlossDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSlossDTh() {
      // Check if tracing is enabled
...
class DualLossUpdater {
 public:
  virtual ~DualLossUpdater() {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSlossDTh mht_0(mht_0_v, 193, "", "./tensorflow/core/kernels/loss.h", "~DualLossUpdater");
}

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc() {
      // Check if tracing is enabled
...
__device__ T impl_sign(T x) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc mht_0(mht_0_v, 200, "", "./tensorflow/core/kernels/training_ops_gpu.cu.cc", "impl_sign");
...
bool to_pointers(bool x) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc mht_1(mht_1_v, 534, "", "./tensorflow/core/kernels/training_ops_gpu.cu.cc", "to_pointers");
 return x; }
template <class T>
typename T::PointerType to_pointers(T& x) {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc mht_2(mht_2_v, 539, "", "./tensorflow/core/kernels/training_ops_gpu.cu.cc", "to_pointers");
...
typename T::ConstPointerType to_pointers(const T& x) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPStraining_ops_gpuDTcuDTcc mht_3(mht_3_v, 546, "", "./tensorflow/core/kernels/training_ops_gpu.cu.cc", "to_pointers");

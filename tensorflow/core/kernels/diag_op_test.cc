...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSdiag_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdiag_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSdiag_op_testDTcc() {
      // Check if tracing is enabled
...
static Graph* Diag(int n, DataType type) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSdiag_op_testDTcc mht_0(mht_0_v, 193, "", "./tensorflow/core/kernels/diag_op_test.cc", "Diag");

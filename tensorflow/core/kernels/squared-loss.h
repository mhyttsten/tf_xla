...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh() {
      // Check if tracing is enabled
...
class SquaredLossUpdater : public DualLossUpdater {
 public:
  ...
  double ComputeUpdatedDual(const int num_loss_partitions, const double label,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh mht_0(mht_0_v, 204, "", "./tensorflow/core/kernels/squared-loss.h", "ComputeUpdatedDual");
  ...
  double ComputeDualLoss(const double current_dual, const double example_label,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh mht_1(mht_1_v, 217, "", "./tensorflow/core/kernels/squared-loss.h", "ComputeDualLoss");
  ...
  double ComputePrimalLoss(const double wx, const double example_label,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh mht_2(mht_2_v, 228, "", "./tensorflow/core/kernels/squared-loss.h", "ComputePrimalLoss");
  ...
  inline double PrimalLossDerivative(const double wx, const double label,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh mht_3(mht_3_v, 237, "", "./tensorflow/core/kernels/squared-loss.h", "PrimalLossDerivative");

    return (wx - label) * example_weight;
  }

  inline double SmoothnessConstant() const final {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh mht_4(mht_4_v, 244, "", "./tensorflow/core/kernels/squared-loss.h", "SmoothnessConstant");
 return 1.0; }

  // Labels don't require conversion for linear regression.
  Status ConvertLabel(float* const example_label) const final {
   std::vector<std::string> mht_5_v;
   MHTracer_DTPStensorflowPScorePSkernelsPSsquaredSLlossDTh mht_5(mht_5_v, 250, "", "./tensorflow/core/kernels/squared-loss.h", "ConvertLabel");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsummary_interfaceDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsummary_interfaceDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsummary_interfaceDTh() {
      // Check if tracing is enabled
...
class SummaryWriterInterface : public ResourceBase {
 public:
  virtual ~SummaryWriterInterface() override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsummary_interfaceDTh mht_0(mht_0_v, 201, "", "./tensorflow/core/kernels/summary_interface.h", "~SummaryWriterInterface");
}

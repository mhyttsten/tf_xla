...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_op_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_op_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_op_testDTcc() {
      // Check if tracing is enabled
...
static Graph* InTopK(int num_targets, int num_classes, T top_k) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSin_topk_op_testDTcc mht_0(mht_0_v, 205, "", "./tensorflow/core/kernels/in_topk_op_test.cc", "InTopK");

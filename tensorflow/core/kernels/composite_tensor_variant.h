...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPScomposite_tensor_variantDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPScomposite_tensor_variantDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPScomposite_tensor_variantDTh() {
      // Check if tracing is enabled
...
class CompositeTensorVariant {
 public:
  ...
  const CompositeTensorVariantMetadata& metadata() const {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPScomposite_tensor_variantDTh mht_0(mht_0_v, 233, "", "./tensorflow/core/kernels/composite_tensor_variant.h", "metadata");
 return *metadata_; }

  // Variant methods.
  string TypeName() const {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSkernelsPScomposite_tensor_variantDTh mht_1(mht_1_v, 239, "", "./tensorflow/core/kernels/composite_tensor_variant.h", "TypeName");
 return kTypeName; }

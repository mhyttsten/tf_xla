...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSbucketize_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbucketize_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSbucketize_opDTcc() {
      // Check if tracing is enabled
...
struct BucketizeFunctor<CPUDevice, T> {
  ...
  static Status Compute(OpKernelContext* context,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbucketize_opDTcc mht_0(mht_0_v, 208, "", "./tensorflow/core/kernels/bucketize_op.cc", "Compute");
...
class BucketizeOp : public OpKernel {
 public:
  explicit BucketizeOp(OpKernelConstruction* context) : OpKernel(context) {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbucketize_opDTcc mht_1(mht_1_v, 228, "", "./tensorflow/core/kernels/bucketize_op.cc", "BucketizeOp");
  ...
  void Compute(OpKernelContext* context) override {
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSbucketize_opDTcc mht_2(mht_2_v, 237, "", "./tensorflow/core/kernels/bucketize_op.cc", "Compute");

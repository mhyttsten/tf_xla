...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsoftmax_op_functorDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsoftmax_op_functorDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsoftmax_op_functorDTh() {
      // Check if tracing is enabled
...
struct SoftmaxEigenImpl {
  static void Compute(const Device& d, typename TTypes<T>::ConstMatrix logits,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsoftmax_op_functorDTh mht_0(mht_0_v, 214, "", "./tensorflow/core/kernels/softmax_op_functor.h", "Compute");

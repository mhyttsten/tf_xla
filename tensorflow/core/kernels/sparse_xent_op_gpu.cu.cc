...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSsparse_xent_op_gpuDTcuDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_xent_op_gpuDTcuDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSsparse_xent_op_gpuDTcuDTcc() {
      // Check if tracing is enabled
...
struct RowMaxReduction<GPUDevice, T> {
  ...
  static inline void Compute(OpKernelContext* ctx,
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSsparse_xent_op_gpuDTcuDTcc mht_0(mht_0_v, 212, "", "./tensorflow/core/kernels/sparse_xent_op_gpu.cu.cc", "Compute");

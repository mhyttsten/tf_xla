...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSkernelsPSpriority_queue_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpriority_queue_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSkernelsPSpriority_queue_opDTcc() {
      // Check if tracing is enabled
...
class PriorityQueueOp : public TypedQueueOp {
 public:
  explicit PriorityQueueOp(OpKernelConstruction* context)
   ...
   MHTracer_DTPStensorflowPScorePSkernelsPSpriority_queue_opDTcc mht_0(mht_0_v, 213, "", "./tensorflow/core/kernels/priority_queue_op.cc", "PriorityQueueOp");

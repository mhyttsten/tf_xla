...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePScommon_runtimePSall_to_allDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePScommon_runtimePSall_to_allDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePScommon_runtimePSall_to_allDTh() {
      // Check if tracing is enabled
...
class AllToAll : public CollectiveImplementationInterface {
 public:
  ...
  Status InitializeCollectiveParams(CollectiveParams* col_params) override {
   ...
   MHTracer_DTPStensorflowPScorePScommon_runtimePSall_to_allDTh mht_0(mht_0_v, 205, "", "./tensorflow/core/common_runtime/all_to_all.h", "InitializeCollectiveParams");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePScommon_runtimePSring_reducerDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePScommon_runtimePSring_reducerDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePScommon_runtimePSring_reducerDTh() {
      // Check if tracing is enabled
...
class RingReducer : public RingAlg {
 public:
  RingReducer() : RingAlg(REDUCTION_COLLECTIVE, "Reduce") {
   ...
   MHTracer_DTPStensorflowPScorePScommon_runtimePSring_reducerDTh mht_0(mht_0_v, 202, "", "./tensorflow/core/common_runtime/ring_reducer.h", "RingReducer");
}

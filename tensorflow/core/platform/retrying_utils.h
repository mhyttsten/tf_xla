...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSretrying_utilsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSretrying_utilsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSretrying_utilsDTh() {
      // Check if tracing is enabled
...
struct RetryConfig {
  RetryConfig(int64_t init_delay_time_us = 100 * 1000,
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSretrying_utilsDTh mht_0(mht_0_v, 198, "", "./tensorflow/core/platform/retrying_utils.h", "RetryConfig");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh() {
      // Check if tracing is enabled
...
class EnvTime {
 public:
  ...
  static uint64 NowMicros() {
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh mht_0(mht_0_v, 213, "", "./tensorflow/core/platform/env_time.h", "NowMicros");
 return NowNanos() / kMicrosToNanos; }

  /// \brief Returns the number of seconds since the Unix epoch.
  static uint64 NowSeconds() {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh mht_1(mht_1_v, 219, "", "./tensorflow/core/platform/env_time.h", "NowSeconds");
 return NowNanos() / kSecondsToNanos; }

  /// \brief A version of NowNanos() that may be overridden by a subclass.
  virtual uint64 GetOverridableNowNanos() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh mht_2(mht_2_v, 225, "", "./tensorflow/core/platform/env_time.h", "GetOverridableNowNanos");
 return NowNanos(); }

  /// \brief A version of NowMicros() that may be overridden by a subclass.
  virtual uint64 GetOverridableNowMicros() const {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh mht_3(mht_3_v, 231, "", "./tensorflow/core/platform/env_time.h", "GetOverridableNowMicros");
  ...
  virtual uint64 GetOverridableNowSeconds() const {
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSenv_timeDTh mht_4(mht_4_v, 239, "", "./tensorflow/core/platform/env_time.h", "GetOverridableNowSeconds");

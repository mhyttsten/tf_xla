...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSdefaultPSmutexDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSdefaultPSmutexDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSdefaultPSmutexDTh() {
      // Check if tracing is enabled
...
std::cv_status condition_variable::wait_for(
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSdefaultPSmutexDTh mht_0(mht_0_v, 201, "", "./tensorflow/core/platform/default/mutex.h", "condition_variable::wait_for");

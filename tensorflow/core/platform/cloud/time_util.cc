...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPScloudPStime_utilDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPScloudPStime_utilDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPScloudPStime_utilDTcc() {
      // Check if tracing is enabled
...
Status ParseRfc3339Time(const string& time, int64_t* mtime_nsec) {
   ...
   mht_0_v.push_back("time: \"" + time + "\"");
   MHTracer_DTPStensorflowPScorePSplatformPScloudPStime_utilDTcc mht_0(mht_0_v, 205, "", "./tensorflow/core/platform/cloud/time_util.cc", "ParseRfc3339Time");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc() {
      // Check if tracing is enabled
...
class TestTime : public EnvTime {
 public:
  uint64 GetOverridableNowNanos() const override {
   ...
   MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc mht_0(mht_0_v, 197, "", "./tensorflow/core/platform/cloud/gcs_throttle_test.cc", "GetOverridableNowNanos");

    return now_micros_ * kMicrosToNanos;
  }

  void SetTime(uint64 now_micros) {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc mht_1(mht_1_v, 204, "", "./tensorflow/core/platform/cloud/gcs_throttle_test.cc", "SetTime");
 now_micros_ = now_micros; }

  void AdvanceSeconds(int64_t secs) {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc mht_2(mht_2_v, 209, "", "./tensorflow/core/platform/cloud/gcs_throttle_test.cc", "AdvanceSeconds");
 now_micros_ += secs * kSecondsToMicros; }
...
class GcsThrottleTest : public ::testing::Test {
 protected:
  GcsThrottleTest() : throttle_(&time_) {
   ...
   MHTracer_DTPStensorflowPScorePSplatformPScloudPSgcs_throttle_testDTcc mht_3(mht_3_v, 220, "", "./tensorflow/core/platform/cloud/gcs_throttle_test.cc", "GcsThrottleTest");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSwindowsPSsubprocessDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSwindowsPSsubprocessDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSwindowsPSsubprocessDTh() {
      // Check if tracing is enabled
...
class SubProcess {
 ...
 private:
  ...
  static bool chan_valid(int chan) {
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSwindowsPSsubprocessDTh mht_0(mht_0_v, 271, "", "./tensorflow/core/platform/windows/subprocess.h", "chan_valid");

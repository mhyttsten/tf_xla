...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSstrong_hashDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSstrong_hashDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSstrong_hashDTh() {
      // Check if tracing is enabled
...
inline uint64 StrongKeyedHash(const tensorflow::uint64 (&key)[2],
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSstrong_hashDTh mht_0(mht_0_v, 208, "", "./tensorflow/core/platform/strong_hash.h", "StrongKeyedHash");

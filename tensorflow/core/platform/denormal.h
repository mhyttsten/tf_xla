...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSdenormalDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSdenormalDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSdenormalDTh() {
      // Check if tracing is enabled
...
class DenormalState {
 public:
  DenormalState(bool flush_to_zero, bool denormals_are_zero)
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSdenormalDTh mht_0(mht_0_v, 198, "", "./tensorflow/core/platform/denormal.h", "DenormalState");
}

  // Output denormals of floating-point operations are flushed to zero.
  inline bool flush_to_zero() const {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSplatformPSdenormalDTh mht_1(mht_1_v, 204, "", "./tensorflow/core/platform/denormal.h", "flush_to_zero");
 return flush_to_zero_; }

  // Input denormals to floating-point operations are treated as zero.
  inline bool denormals_are_zero() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePSplatformPSdenormalDTh mht_2(mht_2_v, 210, "", "./tensorflow/core/platform/denormal.h", "denormals_are_zero");
 return denormals_are_zero_; }

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSprotobuf_internalDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSprotobuf_internalDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSprotobuf_internalDTh() {
      // Check if tracing is enabled
...
Status ParseAny(const google::protobuf::Any& any, T* message,
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSprotobuf_internalDTh mht_0(mht_0_v, 200, "", "./tensorflow/core/platform/protobuf_internal.h", "ParseAny");

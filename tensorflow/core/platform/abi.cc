...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSplatformPSabiDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSabiDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSplatformPSabiDTcc() {
      // Check if tracing is enabled
...
string MaybeAbiDemangle(const char* name) {
   ...
   MHTracer_DTPStensorflowPScorePSplatformPSabiDTcc mht_0(mht_0_v, 210, "", "./tensorflow/core/platform/abi.cc", "MaybeAbiDemangle");

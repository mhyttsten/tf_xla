...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSirPSinterfaces_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSirPSinterfaces_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSirPSinterfaces_testDTcc() {
      // Check if tracing is enabled
...
TEST(TensorFlowRegisterInterface, TestCustomImplementation) {
  ...
  struct CustomRegistryInterface : public TensorFlowRegistryInterfaceBase {
   ...
   MHTracer_DTPStensorflowPScorePSirPSinterfaces_testDTcc mht_0(mht_0_v, 221, "", "./tensorflow/core/ir/interfaces_test.cc", "isStateful");

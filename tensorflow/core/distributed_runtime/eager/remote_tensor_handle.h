...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSdistributed_runtimePSeagerPSremote_tensor_handleDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSdistributed_runtimePSeagerPSremote_tensor_handleDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSdistributed_runtimePSeagerPSremote_tensor_handleDTh() {
      // Check if tracing is enabled
...
struct RemoteTensorHandleInternal {
  explicit RemoteTensorHandleInternal(const RemoteTensorHandle& tensor_handle)
   ...
   MHTracer_DTPStensorflowPScorePSdistributed_runtimePSeagerPSremote_tensor_handleDTh mht_0(mht_0_v, 196, "", "./tensorflow/core/distributed_runtime/eager/remote_tensor_handle.h", "RemoteTensorHandleInternal");
}
  RemoteTensorHandleInternal(int64_t op_id, int32_t output_num)
      : op_id(op_id), output_num(output_num) {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePSdistributed_runtimePSeagerPSremote_tensor_handleDTh mht_1(mht_1_v, 201, "", "./tensorflow/core/distributed_runtime/eager/remote_tensor_handle.h", "RemoteTensorHandleInternal");
}
  int64_t op_id;
  int32 output_num;
};

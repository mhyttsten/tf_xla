...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePStpuPSkernelsPStpu_handle_to_key_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePStpuPSkernelsPStpu_handle_to_key_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePStpuPSkernelsPStpu_handle_to_key_opDTcc() {
      // Check if tracing is enabled
...
class TpuHandleToProtoKeyOp : public OpKernel {
 public:
  explicit TpuHandleToProtoKeyOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
   ...
   MHTracer_DTPStensorflowPScorePStpuPSkernelsPStpu_handle_to_key_opDTcc mht_0(mht_0_v, 198, "", "./tensorflow/core/tpu/kernels/tpu_handle_to_key_op.cc", "TpuHandleToProtoKeyOp");
}
  ...
  void Compute(OpKernelContext* ctx) override {
   ...
   MHTracer_DTPStensorflowPScorePStpuPSkernelsPStpu_handle_to_key_opDTcc mht_1(mht_1_v, 206, "", "./tensorflow/core/tpu/kernels/tpu_handle_to_key_op.cc", "Compute");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSsummaryPSschemaDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSsummaryPSschemaDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSsummaryPSschemaDTcc() {
      // Check if tracing is enabled
...
Status Run(Sqlite* db, const char* sql) {
   ...
   mht_0_v.push_back("sql: \"" + (sql == nullptr ? std::string("nullptr") : std::string((char*)sql)) + "\"");
   MHTracer_DTPStensorflowPScorePSsummaryPSschemaDTcc mht_0(mht_0_v, 192, "", "./tensorflow/core/summary/schema.cc", "Run");
...
Status SetupTensorboardSqliteDb(Sqlite* db) {
   ...
   MHTracer_DTPStensorflowPScorePSsummaryPSschemaDTcc mht_1(mht_1_v, 203, "", "./tensorflow/core/summary/schema.cc", "SetupTensorboardSqliteDb");

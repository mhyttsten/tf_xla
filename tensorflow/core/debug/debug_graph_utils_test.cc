...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSdebugPSdebug_graph_utils_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_graph_utils_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSdebugPSdebug_graph_utils_testDTcc() {
      // Check if tracing is enabled
...
class DebugGraphUtilsTest : public ::testing::Test {
 protected:
  Status ParseDebugOpName(const string& debug_op_name,
   ...
   mht_0_v.push_back("debug_op_name: \"" + debug_op_name + "\"");
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_graph_utils_testDTcc mht_0(mht_0_v, 198, "", "./tensorflow/core/debug/debug_graph_utils_test.cc", "ParseDebugOpName");

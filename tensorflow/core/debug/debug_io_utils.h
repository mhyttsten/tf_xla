...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSdebugPSdebug_io_utilsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_io_utilsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSdebugPSdebug_io_utilsDTh() {
      // Check if tracing is enabled
...
struct DebugWatchAndURLSpec {
  DebugWatchAndURLSpec(const string& watch_key, const string& url,
   ...
   mht_0_v.push_back("watch_key: \"" + watch_key + "\"");
   mht_0_v.push_back("url: \"" + url + "\"");
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_io_utilsDTh mht_0(mht_0_v, 213, "", "./tensorflow/core/debug/debug_io_utils.h", "DebugWatchAndURLSpec");
}
...
class DebugGrpcChannel {
 public:
  ...
  virtual ~DebugGrpcChannel() {
   ...
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_io_utilsDTh mht_1(mht_1_v, 445, "", "./tensorflow/core/debug/debug_io_utils.h", "~DebugGrpcChannel");
}

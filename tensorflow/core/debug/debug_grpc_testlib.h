...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSdebugPSdebug_grpc_testlibDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_grpc_testlibDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSdebugPSdebug_grpc_testlibDTh() {
      // Check if tracing is enabled
...
class TestEventListenerImpl final : public EventListener::Service {
 public:
  TestEventListenerImpl() : stop_requested_(false), stopped_(false) {
   ...
   MHTracer_DTPStensorflowPScorePSdebugPSdebug_grpc_testlibDTh mht_0(mht_0_v, 203, "", "./tensorflow/core/debug/debug_grpc_testlib.h", "TestEventListenerImpl");
}

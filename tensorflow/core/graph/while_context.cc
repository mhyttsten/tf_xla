...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSgraphPSwhile_contextDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSgraphPSwhile_contextDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSgraphPSwhile_contextDTcc() {
      // Check if tracing is enabled
...
WhileContext::WhileContext(StringPiece frame_name,
   ...
   MHTracer_DTPStensorflowPScorePSgraphPSwhile_contextDTcc mht_0(mht_0_v, 200, "", "./tensorflow/core/graph/while_context.cc", "WhileContext::WhileContext");

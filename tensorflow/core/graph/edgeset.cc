...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSgraphPSedgesetDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSgraphPSedgesetDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSgraphPSedgesetDTcc() {
      // Check if tracing is enabled
...
EdgeSet::size_type EdgeSet::erase(key_type key) {
   ...
   MHTracer_DTPStensorflowPScorePSgraphPSedgesetDTcc mht_0(mht_0_v, 221, "", "./tensorflow/core/graph/edgeset.cc", "EdgeSet::erase");

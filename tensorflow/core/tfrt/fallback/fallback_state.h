...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh() {
      // Check if tracing is enabled
...
class FallbackState {
 public:
  ...
  const SessionOptions &session_options() const {
   ...
   MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh mht_0(mht_0_v, 219, "", "./tensorflow/core/tfrt/fallback/fallback_state.h", "session_options");
 return session_options_; }

  const DeviceMgr &device_manager() const {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh mht_1(mht_1_v, 224, "", "./tensorflow/core/tfrt/fallback/fallback_state.h", "device_manager");
 return device_manager_; }

  const DeviceSet &device_set() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh mht_2(mht_2_v, 229, "", "./tensorflow/core/tfrt/fallback/fallback_state.h", "device_set");
 return device_set_; }

  const ProcessFunctionLibraryRuntime &process_function_library_runtime()
      const {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPScorePStfrtPSfallbackPSfallback_stateDTh mht_3(mht_3_v, 235, "", "./tensorflow/core/tfrt/fallback/fallback_state.h", "process_function_library_runtime");

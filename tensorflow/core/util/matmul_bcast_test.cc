...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSutilPSmatmul_bcast_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSutilPSmatmul_bcast_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSutilPSmatmul_bcast_testDTcc() {
      // Check if tracing is enabled
...
string MatMulBCastToStr(const MatMulBCast& b) {
   ...
   MHTracer_DTPStensorflowPScorePSutilPSmatmul_bcast_testDTcc mht_0(mht_0_v, 194, "", "./tensorflow/core/util/matmul_bcast_test.cc", "MatMulBCastToStr");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSutilPSactivation_modeDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSutilPSactivation_modeDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSutilPSactivation_modeDTh() {
      // Check if tracing is enabled
...
inline absl::string_view ToString(ActivationMode mode) {
   ...
   MHTracer_DTPStensorflowPScorePSutilPSactivation_modeDTh mht_0(mht_0_v, 213, "", "./tensorflow/core/util/activation_mode.h", "ToString");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSutilPSexec_on_stall_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSutilPSexec_on_stall_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSutilPSexec_on_stall_testDTcc() {
      // Check if tracing is enabled
...
Chunk* NewChunk(int stall_seconds, std::function<void()> f) {
   ...
   MHTracer_DTPStensorflowPScorePSutilPSexec_on_stall_testDTcc mht_0(mht_0_v, 198, "", "./tensorflow/core/util/exec_on_stall_test.cc", "NewChunk");

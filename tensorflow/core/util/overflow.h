...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSutilPSoverflowDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSutilPSoverflowDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSutilPSoverflowDTh() {
      // Check if tracing is enabled
...
inline int64_t MultiplyWithoutOverflow(const int64_t x, const int64_t y) {
   ...
   MHTracer_DTPStensorflowPScorePSutilPSoverflowDTh mht_0(mht_0_v, 196, "", "./tensorflow/core/util/overflow.h", "MultiplyWithoutOverflow");

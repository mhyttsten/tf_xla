...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSfunctionPSruntime_clientDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSfunctionPSruntime_clientDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSfunctionPSruntime_clientDTh() {
      // Check if tracing is enabled
...
class Runtime {
 public:
  explicit Runtime(EagerContext& eager_ctx) : eager_ctx_(eager_ctx) {
   ...
   MHTracer_DTPStensorflowPScorePSfunctionPSruntime_clientDTh mht_0(mht_0_v, 229, "", "./tensorflow/core/function/runtime_client.h", "Runtime");
}

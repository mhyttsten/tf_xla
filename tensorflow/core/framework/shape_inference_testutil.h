...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSframeworkPSshape_inference_testutilDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSshape_inference_testutilDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSframeworkPSshape_inference_testutilDTh() {
      // Check if tracing is enabled
...
struct ShapeInferenceTestOp {
  ...
  explicit ShapeInferenceTestOp(StringPiece name) : name(string(name)) {
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSshape_inference_testutilDTh mht_0(mht_0_v, 205, "", "./tensorflow/core/framework/shape_inference_testutil.h", "ShapeInferenceTestOp");
}
  string name;
  NodeDef node_def;
...
class ShapeInferenceTestutil {
 ...
 private:
  ShapeInferenceTestutil() {
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSshape_inference_testutilDTh mht_1(mht_1_v, 246, "", "./tensorflow/core/framework/shape_inference_testutil.h", "ShapeInferenceTestutil");
}

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSframeworkPSfake_inputDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSfake_inputDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSframeworkPSfake_inputDTh() {
      // Check if tracing is enabled
...
inline FakeInputFunctor FakeInput(std::initializer_list<DataType> dts) {
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSfake_inputDTh mht_0(mht_0_v, 203, "", "./tensorflow/core/framework/fake_input.h", "FakeInput");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSframeworkPSbounds_checkDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSbounds_checkDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSframeworkPSbounds_checkDTh() {
      // Check if tracing is enabled
...
EIGEN_ALWAYS_INLINE EIGEN_DEVICE_FUNC const T SubtleMustCopy(const T &x) {
   ...
   MHTracer_DTPStensorflowPScorePSframeworkPSbounds_checkDTh mht_0(mht_0_v, 214, "", "./tensorflow/core/framework/bounds_check.h", "SubtleMustCopy");

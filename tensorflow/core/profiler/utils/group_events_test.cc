...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSgroup_events_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSgroup_events_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSgroup_events_testDTcc() {
      // Check if tracing is enabled
...
TEST(GroupEventsTest, EagerOpTest) {
  ...
  auto is_eager = [](const XEventVisitor& event) {
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSgroup_events_testDTcc mht_0(mht_0_v, 516, "", "./tensorflow/core/profiler/utils/group_events_test.cc", "lambda");

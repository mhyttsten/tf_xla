...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc() {
      // Check if tracing is enabled
...
double GetCappedPerf(double perf, uint64 time, double rate_limit) {
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc mht_0(mht_0_v, 204, "", "./tensorflow/core/profiler/utils/op_utils.cc", "GetCappedPerf");
...
void HostOpMetricsDbBuilder::EnterOp(absl::string_view name,
   ...
   mht_1_v.push_back("category: \"" + std::string(category.data(), category.size()) + "\"");
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc mht_1(mht_1_v, 219, "", "./tensorflow/core/profiler/utils/op_utils.cc", "HostOpMetricsDbBuilder::EnterOp");
...
void HostOpMetricsDbBuilder::EnterHostInfeedEnqueue(
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc mht_2(mht_2_v, 236, "", "./tensorflow/core/profiler/utils/op_utils.cc", "HostOpMetricsDbBuilder::EnterHostInfeedEnqueue");
...
void DeviceOpMetricsDbBuilder::EnterOp(
   ...
   mht_3_v.push_back("category: \"" + std::string(category.data(), category.size()) + "\"");
   mht_3_v.push_back("provenance: \"" + std::string(provenance.data(), provenance.size()) + "\"");
   MHTracer_DTPStensorflowPScorePSprofilerPSutilsPSop_utilsDTcc mht_3(mht_3_v, 264, "", "./tensorflow/core/profiler/utils/op_utils.cc", "DeviceOpMetricsDbBuilder::EnterOp");

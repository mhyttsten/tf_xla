...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_step_statsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_step_statsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_step_statsDTcc() {
      // Check if tracing is enabled
...
GpuEventType ParseMemcpyName(absl::string_view memcpy_name) {
   ...
   mht_0_v.push_back("memcpy_name: \"" + std::string(memcpy_name.data(), memcpy_name.size()) + "\"");
   MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_step_statsDTcc mht_0(mht_0_v, 221, "", "./tensorflow/core/profiler/convert/xplane_to_step_stats.cc", "ParseMemcpyName");
...
void SetNodeTimes(const XEventVisitor& event, NodeExecStats* ns) {
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_step_statsDTcc mht_1(mht_1_v, 234, "", "./tensorflow/core/profiler/convert/xplane_to_step_stats.cc", "SetNodeTimes");
...
void ConvertGpuXSpaceToStepStats(const XSpace& xspace, StepStats* step_stats) {
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_step_statsDTcc mht_2(mht_2_v, 246, "", "./tensorflow/core/profiler/convert/xplane_to_step_stats.cc", "ConvertGpuXSpaceToStepStats");

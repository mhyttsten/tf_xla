...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_op_stats_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_op_stats_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_op_stats_testDTcc() {
      // Check if tracing is enabled
...
void BuildXSpaceForTest(XSpace& xspace, absl::string_view hostname) {
   ...
   mht_0_v.push_back("hostname: \"" + std::string(hostname.data(), hostname.size()) + "\"");
   MHTracer_DTPStensorflowPScorePSprofilerPSconvertPSxplane_to_op_stats_testDTcc mht_0(mht_0_v, 373, "", "./tensorflow/core/profiler/convert/xplane_to_op_stats_test.cc", "BuildXSpaceForTest");

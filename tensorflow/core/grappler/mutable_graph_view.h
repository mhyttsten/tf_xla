...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSgrapplerPSmutable_graph_viewDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSgrapplerPSmutable_graph_viewDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSgrapplerPSmutable_graph_viewDTh() {
      // Check if tracing is enabled
...
class MutableGraphView : public internal::GraphViewInternal<GraphDef, NodeDef> {
 public:
  explicit MutableGraphView(GraphDef* graph) : GraphViewInternal(graph) {
   ...
   MHTracer_DTPStensorflowPScorePSgrapplerPSmutable_graph_viewDTh mht_0(mht_0_v, 214, "", "./tensorflow/core/grappler/mutable_graph_view.h", "MutableGraphView");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSgrapplerPSgraph_topology_view_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSgrapplerPSgraph_topology_view_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSgrapplerPSgraph_topology_view_testDTcc() {
      // Check if tracing is enabled
...
class GraphTopologyViewTest : public ::testing::Test {
 protected:
  ...
  static GraphDef CreateGraph(const std::vector<NodeConfig>& nodes) {
   ...
   MHTracer_DTPStensorflowPScorePSgrapplerPSgraph_topology_view_testDTcc mht_0(mht_0_v, 198, "", "./tensorflow/core/grappler/graph_topology_view_test.cc", "CreateGraph");

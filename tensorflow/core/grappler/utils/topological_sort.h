...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSgrapplerPSutilsPStopological_sortDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSgrapplerPSutilsPStopological_sortDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSgrapplerPSutilsPStopological_sortDTh() {
      // Check if tracing is enabled
...
struct TopologicalDependency {
  TopologicalDependency(const NodeDef* from, const NodeDef* to)
   ...
   MHTracer_DTPStensorflowPScorePSgrapplerPSutilsPStopological_sortDTh mht_0(mht_0_v, 199, "", "./tensorflow/core/grappler/utils/topological_sort.h", "TopologicalDependency");
}
  const NodeDef* from;
  const NodeDef* to;
};

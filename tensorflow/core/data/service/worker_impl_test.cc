...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScorePSdataPSservicePSworker_impl_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScorePSdataPSservicePSworker_impl_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScorePSdataPSservicePSworker_impl_testDTcc() {
      // Check if tracing is enabled
...
class LocalWorkersTest : public ::testing::Test {
 protected:
  void SetUp() override {
   ...
   MHTracer_DTPStensorflowPScorePSdataPSservicePSworker_impl_testDTcc mht_0(mht_0_v, 203, "", "./tensorflow/core/data/service/worker_impl_test.cc", "SetUp");

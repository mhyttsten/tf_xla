...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSpythonPSutilPSstack_traceDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSpythonPSutilPSstack_traceDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSpythonPSutilPSstack_traceDTcc() {
      // Check if tracing is enabled
...
const char* GetPythonString(PyObject* o) {
   ...
   MHTracer_DTPStensorflowPSpythonPSutilPSstack_traceDTcc mht_0(mht_0_v, 197, "", "./tensorflow/python/util/stack_trace.cc", "GetPythonString");
...
std::vector<StackFrame> StackTrace::ToStackFrames(
   ...
   MHTracer_DTPStensorflowPSpythonPSutilPSstack_traceDTcc mht_1(mht_1_v, 218, "", "./tensorflow/python/util/stack_trace.cc", "StackTrace::ToStackFrames");
...
StackTrace* StackTraceManager::Get(int id) {
   ...
   MHTracer_DTPStensorflowPSpythonPSutilPSstack_traceDTcc mht_2(mht_2_v, 255, "", "./tensorflow/python/util/stack_trace.cc", "StackTraceManager::Get");

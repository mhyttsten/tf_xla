...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSpythonPSgrapplerPSmodel_analyzerDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSpythonPSgrapplerPSmodel_analyzerDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSpythonPSgrapplerPSmodel_analyzerDTcc() {
      // Check if tracing is enabled
...
ModelAnalyzer::ModelAnalyzer(const GrapplerItem& item) : item_(item) {
   ...
   MHTracer_DTPStensorflowPSpythonPSgrapplerPSmodel_analyzerDTcc mht_0(mht_0_v, 196, "", "./tensorflow/python/grappler/model_analyzer.cc", "ModelAnalyzer::ModelAnalyzer");
}

Status ModelAnalyzer::GenerateReport(bool debug, bool assume_valid_feeds,
                                     std::ostream& os) {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSpythonPSgrapplerPSmodel_analyzerDTcc mht_1(mht_1_v, 202, "", "./tensorflow/python/grappler/model_analyzer.cc", "ModelAnalyzer::GenerateReport");
...
void ModelAnalyzer::PrintNodeInfo(const NodeDef* node,
   ...
   MHTracer_DTPStensorflowPSpythonPSgrapplerPSmodel_analyzerDTcc mht_2(mht_2_v, 221, "", "./tensorflow/python/grappler/model_analyzer.cc", "ModelAnalyzer::PrintNodeInfo");

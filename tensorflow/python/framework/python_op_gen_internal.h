...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_op_gen_internalDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_op_gen_internalDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_op_gen_internalDTh() {
      // Check if tracing is enabled
...
class ParamNames {
 public:
  ...
  ParamNames(const string& name, const string& rename_to) : name_(name) {
   ...
   mht_0_v.push_back("rename_to: \"" + rename_to + "\"");
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_op_gen_internalDTh mht_0(mht_0_v, 224, "", "./tensorflow/python/framework/python_op_gen_internal.h", "ParamNames");
  ...
  string GetName() const {
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_op_gen_internalDTh mht_1(mht_1_v, 232, "", "./tensorflow/python/framework/python_op_gen_internal.h", "GetName");
 return name_; }

  // Get the name to rename the parameter to. Note that AvoidPythonReserved
  // has already been applied.
  string GetRenameTo() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_op_gen_internalDTh mht_2(mht_2_v, 239, "", "./tensorflow/python/framework/python_op_gen_internal.h", "GetRenameTo");
 return rename_to_; }

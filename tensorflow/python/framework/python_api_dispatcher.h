...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh() {
      // Check if tracing is enabled
...
class PyTypeChecker {
 public:
  ...
  virtual ~PyTypeChecker() {
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh mht_0(mht_0_v, 337, "", "./tensorflow/python/framework/python_api_dispatcher.h", "~PyTypeChecker");
}
...
class PyInstanceChecker : public PyTypeChecker {
 public:
  ...
  size_t cache_size() const {
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh mht_1(mht_1_v, 388, "", "./tensorflow/python/framework/python_api_dispatcher.h", "cache_size");
...
class PyListChecker : public PyTypeChecker {
 public:
  explicit PyListChecker(PyTypeChecker_ptr element_type)
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh mht_2(mht_2_v, 414, "", "./tensorflow/python/framework/python_api_dispatcher.h", "PyListChecker");
}
  MatchType Check(PyObject* value) override;
  int cost() const override;
  std::string DebugString() const override;
...
class PyUnionChecker : public PyTypeChecker {
 public:
  explicit PyUnionChecker(std::vector<PyTypeChecker_ptr> options)
   ...
   MHTracer_DTPStensorflowPSpythonPSframeworkPSpython_api_dispatcherDTh mht_3(mht_3_v, 431, "", "./tensorflow/python/framework/python_api_dispatcher.h", "PyUnionChecker");
}
  MatchType Check(PyObject* value) override;
  int cost() const override;
  std::string DebugString() const override;

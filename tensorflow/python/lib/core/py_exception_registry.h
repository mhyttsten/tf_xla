...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSpythonPSlibPScorePSpy_exception_registryDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSpythonPSlibPScorePSpy_exception_registryDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSpythonPSlibPScorePSpy_exception_registryDTh() {
      // Check if tracing is enabled
...
class PyExceptionRegistry {
 public:
  ...
  static inline PyObject* Lookup(error::Code code) {
   ...
   MHTracer_DTPStensorflowPSpythonPSlibPScorePSpy_exception_registryDTh mht_0(mht_0_v, 229, "", "./tensorflow/python/lib/core/py_exception_registry.h", "Lookup");

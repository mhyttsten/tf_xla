...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSpythonPSlibPScorePSpybind11_protoDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSpythonPSlibPScorePSpybind11_protoDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSpythonPSlibPScorePSpybind11_protoDTh() {
      // Check if tracing is enabled
...
inline void CheckProtoType(const pybind11::handle& py_object,
   ...
   mht_0_v.push_back("expected_proto_type: \"" + expected_proto_type + "\"");
   MHTracer_DTPStensorflowPSpythonPSlibPScorePSpybind11_protoDTh mht_0(mht_0_v, 195, "", "./tensorflow/python/lib/core/pybind11_proto.h", "CheckProtoType");

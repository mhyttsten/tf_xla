...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSfile_utilsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSfile_utilsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSfile_utilsDTcc() {
      // Check if tracing is enabled
...
Status LoadTextOrBinaryGraphFile(const string& file_name, GraphDef* graph_def) {
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSfile_utilsDTcc mht_0(mht_0_v, 193, "", "./tensorflow/tools/graph_transforms/file_utils.cc", "LoadTextOrBinaryGraphFile");

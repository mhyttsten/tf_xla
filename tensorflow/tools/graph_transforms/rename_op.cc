...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_opDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_opDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_opDTcc() {
      // Check if tracing is enabled
...
Status RenameOp(const GraphDef& input_graph_def,
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_opDTcc mht_0(mht_0_v, 200, "", "./tensorflow/tools/graph_transforms/rename_op.cc", "RenameOp");

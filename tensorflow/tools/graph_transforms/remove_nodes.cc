...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSremove_nodesDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSremove_nodesDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSremove_nodesDTcc() {
      // Check if tracing is enabled
...
Status RemoveNodes(const GraphDef& input_graph_def,
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSremove_nodesDTcc mht_0(mht_0_v, 201, "", "./tensorflow/tools/graph_transforms/remove_nodes.cc", "RemoveNodes");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_nodeDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_nodeDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_nodeDTcc() {
      // Check if tracing is enabled
...
Status RenameNode(const GraphDef& input_graph_def,
   ...
   MHTracer_DTPStensorflowPStoolsPSgraph_transformsPSrename_nodeDTcc mht_0(mht_0_v, 196, "", "./tensorflow/tools/graph_transforms/rename_node.cc", "RenameNode");

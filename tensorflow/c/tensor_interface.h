...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScPStensor_interfaceDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScPStensor_interfaceDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScPStensor_interfaceDTh() {
      // Check if tracing is enabled
...
class AbstractTensorInterface {
 ...
 protected:
  virtual ~AbstractTensorInterface() {
   ...
   MHTracer_DTPStensorflowPScPStensor_interfaceDTh mht_0(mht_0_v, 225, "", "./tensorflow/c/tensor_interface.h", "~AbstractTensorInterface");
}
};

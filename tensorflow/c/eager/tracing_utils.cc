...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScPSeagerPStracing_utilsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScPSeagerPStracing_utilsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScPSeagerPStracing_utilsDTcc() {
      // Check if tracing is enabled
...
Status MaybeSetOpName(AbstractOperation* op, const char* op_name) {
   ...
   MHTracer_DTPStensorflowPScPSeagerPStracing_utilsDTcc mht_0(mht_0_v, 195, "", "./tensorflow/c/eager/tracing_utils.cc", "MaybeSetOpName");

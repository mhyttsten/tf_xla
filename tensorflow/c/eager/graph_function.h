...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScPSeagerPSgraph_functionDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScPSeagerPSgraph_functionDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScPSeagerPSgraph_functionDTh() {
      // Check if tracing is enabled
...
class GraphFunction : public AbstractFunction {
 public:
  ...
  static bool classof(const AbstractFunction* ptr) {
   ...
   MHTracer_DTPStensorflowPScPSeagerPSgraph_functionDTh mht_0(mht_0_v, 203, "", "./tensorflow/c/eager/graph_function.h", "classof");

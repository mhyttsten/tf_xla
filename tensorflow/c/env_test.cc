...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScPSenv_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScPSenv_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScPSenv_testDTcc() {
      // Check if tracing is enabled
...
void SomeThreadFunc(void* data) {
   ...
   MHTracer_DTPStensorflowPScPSenv_testDTcc mht_0(mht_0_v, 279, "", "./tensorflow/c/env_test.cc", "SomeThreadFunc");

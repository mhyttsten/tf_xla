...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSbucketize_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSbucketize_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSbucketize_testDTcc() {
      // Check if tracing is enabled
...
class BucketizeOpModel : public SingleOpModel {
 public:
  BucketizeOpModel(const TensorData& input,
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSbucketize_testDTcc mht_0(mht_0_v, 202, "", "./tensorflow/lite/kernels/bucketize_test.cc", "BucketizeOpModel");
  ...
  int input() {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSbucketize_testDTcc mht_1(mht_1_v, 218, "", "./tensorflow/lite/kernels/bucketize_test.cc", "input");
 return input_; }
  const std::vector<float>& boundaries() {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSbucketize_testDTcc mht_2(mht_2_v, 222, "", "./tensorflow/lite/kernels/bucketize_test.cc", "boundaries");
 return boundaries_; }

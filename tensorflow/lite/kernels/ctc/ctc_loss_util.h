...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSctcPSctc_loss_utilDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSctcPSctc_loss_utilDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSctcPSctc_loss_utilDTh() {
      // Check if tracing is enabled
...
inline float LogSumExp(float log_prob_1, float log_prob_2) {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSctcPSctc_loss_utilDTh mht_0(mht_0_v, 203, "", "./tensorflow/lite/kernels/ctc/ctc_loss_util.h", "LogSumExp");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh() {
      // Check if tracing is enabled
...
class Spectrogram {
 public:
  Spectrogram() : initialized_(false) {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh mht_0(mht_0_v, 214, "", "./tensorflow/lite/kernels/internal/spectrogram.h", "Spectrogram");
}
  ~Spectrogram() {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh mht_1(mht_1_v, 218, "", "./tensorflow/lite/kernels/internal/spectrogram.h", "~Spectrogram");
}
  ...
  const std::vector<double>& GetWindow() const {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh mht_2(mht_2_v, 255, "", "./tensorflow/lite/kernels/internal/spectrogram.h", "GetWindow");
 return window_; }

  // Return the number of frequency channels in the spectrogram.
  int output_frequency_channels() const {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSinternalPSspectrogramDTh mht_3(mht_3_v, 261, "", "./tensorflow/lite/kernels/internal/spectrogram.h", "output_frequency_channels");

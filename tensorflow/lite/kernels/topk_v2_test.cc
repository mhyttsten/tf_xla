...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPStopk_v2_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPStopk_v2_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPStopk_v2_testDTcc() {
      // Check if tracing is enabled
...
class TopKV2OpModel : public SingleOpModel {
 public:
  TopKV2OpModel(int top_k, std::initializer_list<int> input_shape,
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPStopk_v2_testDTcc mht_0(mht_0_v, 209, "", "./tensorflow/lite/kernels/topk_v2_test.cc", "TopKV2OpModel");

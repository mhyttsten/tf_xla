...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc() {
      // Check if tracing is enabled
...
class ShapeOpModel : public SingleOpModel {
 public:
  ShapeOpModel(std::initializer_list<int> input_shape, TensorType input_type,
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc mht_0(mht_0_v, 205, "", "./tensorflow/lite/kernels/shape_test.cc", "ShapeOpModel");
  ...
  TfLiteStatus InvokeWithResult() {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc mht_1(mht_1_v, 216, "", "./tensorflow/lite/kernels/shape_test.cc", "InvokeWithResult");
 return interpreter_->Invoke(); }

  int input() {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc mht_2(mht_2_v, 221, "", "./tensorflow/lite/kernels/shape_test.cc", "input");
 return input_; }

  int32_t GetOutputSize() {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc mht_3(mht_3_v, 226, "", "./tensorflow/lite/kernels/shape_test.cc", "GetOutputSize");
 return GetTensorSize(output_); }
  std::vector<T> GetOutput() { return ExtractVector<T>(output_); }
  std::vector<int> GetOutputShape() { return GetTensorShape(output_); }
  TfLiteAllocationType GetOutputAllocationType() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshape_testDTcc mht_4(mht_4_v, 232, "", "./tensorflow/lite/kernels/shape_test.cc", "GetOutputAllocationType");

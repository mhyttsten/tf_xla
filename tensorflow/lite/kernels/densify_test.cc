...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSdensify_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSdensify_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSdensify_testDTcc() {
      // Check if tracing is enabled
...
class DensifyOpModel : public SingleOpModel {
 public:
  DensifyOpModel(const TensorData& input, const std::vector<T>& input_data,
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSdensify_testDTcc mht_0(mht_0_v, 214, "", "./tensorflow/lite/kernels/densify_test.cc", "DensifyOpModel");

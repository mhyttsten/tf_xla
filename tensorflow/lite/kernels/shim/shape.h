...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh() {
      // Check if tracing is enabled
...
class Shape {
 public:
  ...
  inline bool has_value() const {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh mht_0(mht_0_v, 218, "", "./tensorflow/lite/kernels/shim/shape.h", "has_value");
 return has_value_; }
  inline ValueType& value() {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh mht_1(mht_1_v, 222, "", "./tensorflow/lite/kernels/shim/shape.h", "value");
 return value_; }
  inline const ValueType& value() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh mht_2(mht_2_v, 226, "", "./tensorflow/lite/kernels/shim/shape.h", "value");
 return value_; }
  ValueType* operator->() { return &value_; }
  const ValueType* operator->() const { return &value_; }
  ValueType& operator*() {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh mht_3(mht_3_v, 232, "", "./tensorflow/lite/kernels/shim/shape.h", "*");
 return value_; }
  const ValueType& operator*() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh mht_4(mht_4_v, 236, "", "./tensorflow/lite/kernels/shim/shape.h", "*");
 return value_; }
  ...
  const int Rank() const {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSshimPSshapeDTh mht_5(mht_5_v, 244, "", "./tensorflow/lite/kernels/shim/shape.h", "Rank");
 return has_value_ ? value_.size() : kUnknownRank; }

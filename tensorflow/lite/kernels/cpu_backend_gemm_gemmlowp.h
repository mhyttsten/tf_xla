...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPScpu_backend_gemm_gemmlowpDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPScpu_backend_gemm_gemmlowpDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPScpu_backend_gemm_gemmlowpDTh() {
      // Check if tracing is enabled
...
struct GemmImplUsingGemmlowp<
  ...
  static void Run(
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPScpu_backend_gemm_gemmlowpDTh mht_0(mht_0_v, 256, "", "./tensorflow/lite/kernels/cpu_backend_gemm_gemmlowp.h", "Run");
...
struct GemmImplUsingGemmlowp<LhsScalar, RhsScalar, AccumScalar, DstScalar,
  ...
  static void Run(
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPScpu_backend_gemm_gemmlowpDTh mht_1(mht_1_v, 316, "", "./tensorflow/lite/kernels/cpu_backend_gemm_gemmlowp.h", "Run");

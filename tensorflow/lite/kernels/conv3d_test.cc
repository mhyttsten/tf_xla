...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc() {
      // Check if tracing is enabled
...
class Conv3dOpModel : public SingleOpModel {
 public:
  Conv3dOpModel(const TensorData& input, const TensorData& filter,
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc mht_0(mht_0_v, 207, "", "./tensorflow/lite/kernels/conv3d_test.cc", "Conv3dOpModel");
  ...
  Conv3dOpModel(const TensorData& input, const TensorData& filter,
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc mht_1(mht_1_v, 230, "", "./tensorflow/lite/kernels/conv3d_test.cc", "Conv3dOpModel");
  ...
  void SetFilter(std::vector<float> f) {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc mht_2(mht_2_v, 246, "", "./tensorflow/lite/kernels/conv3d_test.cc", "SetFilter");
 PopulateTensor(filter_, f); }

  void SetBias(std::initializer_list<float> f) {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc mht_3(mht_3_v, 251, "", "./tensorflow/lite/kernels/conv3d_test.cc", "SetBias");
 PopulateTensor(bias_, f); }

  void SetInput(std::vector<float> data) {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSconv3d_testDTcc mht_4(mht_4_v, 256, "", "./tensorflow/lite/kernels/conv3d_test.cc", "SetInput");

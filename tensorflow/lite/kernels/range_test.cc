...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc() {
      // Check if tracing is enabled
...
class RangeOpModel : public SingleOpModel {
 public:
  explicit RangeOpModel(const TensorType& dtype) {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc mht_0(mht_0_v, 200, "", "./tensorflow/lite/kernels/range_test.cc", "RangeOpModel");
  ...
  int start() {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc mht_1(mht_1_v, 213, "", "./tensorflow/lite/kernels/range_test.cc", "start");
 return start_; }
  int limit() {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc mht_2(mht_2_v, 217, "", "./tensorflow/lite/kernels/range_test.cc", "limit");
 return limit_; }
  int delta() {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSrange_testDTcc mht_3(mht_3_v, 221, "", "./tensorflow/lite/kernels/range_test.cc", "delta");
 return delta_; }

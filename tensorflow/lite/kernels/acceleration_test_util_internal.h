...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh() {
      // Check if tracing is enabled
...
class ConfigurationEntry {
 public:
  ConfigurationEntry(const std::string& test_id_rex, T test_config,
   ...
   mht_0_v.push_back("test_id_rex: \"" + test_id_rex + "\"");
   MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh mht_0(mht_0_v, 215, "", "./tensorflow/lite/kernels/acceleration_test_util_internal.h", "ConfigurationEntry");
}

  bool Matches(const std::string& test_id) {
   std::vector<std::string> mht_1_v;
   mht_1_v.push_back("test_id: \"" + test_id + "\"");
   MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh mht_1(mht_1_v, 221, "", "./tensorflow/lite/kernels/acceleration_test_util_internal.h", "Matches");

    return RE2::FullMatch(test_id, test_id_rex_);
  }
  bool IsDenylistEntry() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh mht_2(mht_2_v, 227, "", "./tensorflow/lite/kernels/acceleration_test_util_internal.h", "IsDenylistEntry");
 return is_denylist_; }
  const T& TestConfig() const {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh mht_3(mht_3_v, 231, "", "./tensorflow/lite/kernels/acceleration_test_util_internal.h", "TestConfig");
 return test_config_; }

  const std::string& TestIdRex() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSacceleration_test_util_internalDTh mht_4(mht_4_v, 236, "", "./tensorflow/lite/kernels/acceleration_test_util_internal.h", "TestIdRex");
 return test_id_rex_; }

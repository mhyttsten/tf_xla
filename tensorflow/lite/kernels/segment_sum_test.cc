...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSsegment_sum_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSsegment_sum_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSsegment_sum_testDTcc() {
      // Check if tracing is enabled
...
class SegmentSumOpModel : public SingleOpModel {
 public:
  SegmentSumOpModel(const TensorData& data, const TensorData& segment_ids) {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSsegment_sum_testDTcc mht_0(mht_0_v, 200, "", "./tensorflow/lite/kernels/segment_sum_test.cc", "SegmentSumOpModel");
  ...
  int data() const {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSsegment_sum_testDTcc mht_1(mht_1_v, 211, "", "./tensorflow/lite/kernels/segment_sum_test.cc", "data");
 return data_id_; }
  int segment_ids() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSsegment_sum_testDTcc mht_2(mht_2_v, 215, "", "./tensorflow/lite/kernels/segment_sum_test.cc", "segment_ids");
 return segment_ids_id_; }

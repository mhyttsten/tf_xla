...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSkernelsPSzeros_like_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSzeros_like_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSkernelsPSzeros_like_testDTcc() {
      // Check if tracing is enabled
...
class ZerosLikeOpModel : public SingleOpModel {
 public:
  explicit ZerosLikeOpModel(const TensorData& input) {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSzeros_like_testDTcc mht_0(mht_0_v, 202, "", "./tensorflow/lite/kernels/zeros_like_test.cc", "ZerosLikeOpModel");
  ...
  int input() {
   ...
   MHTracer_DTPStensorflowPSlitePSkernelsPSzeros_like_testDTcc mht_1(mht_1_v, 213, "", "./tensorflow/lite/kernels/zeros_like_test.cc", "input");
 return input_; }
  int output() {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePSkernelsPSzeros_like_testDTcc mht_2(mht_2_v, 217, "", "./tensorflow/lite/kernels/zeros_like_test.cc", "output");
 return output_; }

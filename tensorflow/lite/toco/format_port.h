...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh() {
      // Check if tracing is enabled
...
T IdentityOrConvertStringToRaw(T foo) {
   ...
   MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh mht_0(mht_0_v, 199, "", "./tensorflow/lite/toco/format_port.h", "IdentityOrConvertStringToRaw");
...
inline const char* IdentityOrConvertStringToRaw(const std::string& foo) {
   ...
   mht_1_v.push_back("foo: \"" + foo + "\"");
   MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh mht_1(mht_1_v, 208, "", "./tensorflow/lite/toco/format_port.h", "IdentityOrConvertStringToRaw");
...
inline void AppendFHelper(std::string* destination, const char* fmt) {
   ...
   mht_2_v.push_back("fmt: \"" + (fmt == nullptr ? std::string("nullptr") : std::string((char*)fmt)) + "\"");
   MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh mht_2(mht_2_v, 224, "", "./tensorflow/lite/toco/format_port.h", "AppendFHelper");
...
inline std::string StringF(const char* fmt, Args&&... args) {
   ...
   mht_3_v.push_back("fmt: \"" + (fmt == nullptr ? std::string("nullptr") : std::string((char*)fmt)) + "\"");
   MHTracer_DTPStensorflowPSlitePStocoPSformat_portDTh mht_3(mht_3_v, 243, "", "./tensorflow/lite/toco/format_port.h", "StringF");

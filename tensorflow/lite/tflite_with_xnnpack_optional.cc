...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStflite_with_xnnpack_optionalDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStflite_with_xnnpack_optionalDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStflite_with_xnnpack_optionalDTcc() {
      // Check if tracing is enabled
...
TfLiteDelegatePtr MaybeCreateXNNPACKDelegate(int num_threads) {
   ...
   MHTracer_DTPStensorflowPSlitePStflite_with_xnnpack_optionalDTcc mht_0(mht_0_v, 217, "", "./tensorflow/lite/tflite_with_xnnpack_optional.cc", "MaybeCreateXNNPACKDelegate");

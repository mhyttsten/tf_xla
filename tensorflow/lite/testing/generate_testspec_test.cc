...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStestingPSgenerate_testspec_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStestingPSgenerate_testspec_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStestingPSgenerate_testspec_testDTcc() {
      // Check if tracing is enabled
...
TEST(GenerateRandomTensor, FloatValue) {
  ...
  auto random_func = [&]() {
   ...
   MHTracer_DTPStensorflowPSlitePStestingPSgenerate_testspec_testDTcc mht_0(mht_0_v, 197, "", "./tensorflow/lite/testing/generate_testspec_test.cc", "lambda");

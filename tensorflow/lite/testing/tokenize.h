...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStestingPStokenizeDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStestingPStokenizeDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStestingPStokenizeDTh() {
      // Check if tracing is enabled
...
class TokenProcessor {
 public:
  virtual ~TokenProcessor() {
   ...
   MHTracer_DTPStensorflowPSlitePStestingPStokenizeDTh mht_0(mht_0_v, 196, "", "./tensorflow/lite/testing/tokenize.h", "~TokenProcessor");
}

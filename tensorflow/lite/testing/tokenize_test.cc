...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStestingPStokenize_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStestingPStokenize_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStestingPStokenize_testDTcc() {
      // Check if tracing is enabled
...
class TokenCollector : public TokenProcessor {
 public:
  void ConsumeToken(std::string* token) override {
   ...
   MHTracer_DTPStensorflowPSlitePStestingPStokenize_testDTcc mht_0(mht_0_v, 198, "", "./tensorflow/lite/testing/tokenize_test.cc", "ConsumeToken");
 tokens_.push_back(*token); }
  const std::vector<std::string>& Tokens() {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSlitePStestingPStokenize_testDTcc mht_1(mht_1_v, 202, "", "./tensorflow/lite/testing/tokenize_test.cc", "Tokens");
 return tokens_; }

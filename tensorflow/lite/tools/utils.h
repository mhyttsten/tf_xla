...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStoolsPSutilsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSutilsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStoolsPSutilsDTh() {
      // Check if tracing is enabled
...
struct InputTensorData {
  InputTensorData() : data(nullptr, nullptr) {
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSutilsDTh mht_0(mht_0_v, 199, "", "./tensorflow/lite/tools/utils.h", "InputTensorData");
}

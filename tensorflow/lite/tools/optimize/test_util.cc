...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStoolsPSoptimizePStest_utilDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSoptimizePStest_utilDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStoolsPSoptimizePStest_utilDTcc() {
      // Check if tracing is enabled
...
int FailOnErrorReporter::Report(const char* format, va_list args) {
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSoptimizePStest_utilDTcc mht_0(mht_0_v, 255, "", "./tensorflow/lite/tools/optimize/test_util.cc", "FailOnErrorReporter::Report");

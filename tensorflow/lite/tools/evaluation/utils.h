...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStoolsPSevaluationPSutilsDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSevaluationPSutilsDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStoolsPSevaluationPSutilsDTh() {
      // Check if tracing is enabled
...
inline TfLiteStatus GetSortedFileNames(const std::string& directory,
   ...
   mht_0_v.push_back("directory: \"" + directory + "\"");
   MHTracer_DTPStensorflowPSlitePStoolsPSevaluationPSutilsDTh mht_0(mht_0_v, 234, "", "./tensorflow/lite/tools/evaluation/utils.h", "GetSortedFileNames");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh() {
      // Check if tracing is enabled
...
class LoggingWrapper {
 public:
  ...
  explicit LoggingWrapper(LogSeverity severity)
   ...
   MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh mht_0(mht_0_v, 216, "", "./tensorflow/lite/tools/logging.h", "LoggingWrapper");
}
  LoggingWrapper(LogSeverity severity, bool log)
      : severity_(severity), should_log_(log) {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh mht_1(mht_1_v, 221, "", "./tensorflow/lite/tools/logging.h", "LoggingWrapper");
}
  std::stringstream& Stream() {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh mht_2(mht_2_v, 225, "", "./tensorflow/lite/tools/logging.h", "Stream");
 return stream_; }
  ~LoggingWrapper() {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPSlitePStoolsPSloggingDTh mht_3(mht_3_v, 229, "", "./tensorflow/lite/tools/logging.h", "~LoggingWrapper");

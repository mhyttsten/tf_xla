...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePScorePSapiPSverifierDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePScorePSapiPSverifierDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePScorePSapiPSverifierDTh() {
      // Check if tracing is enabled
...
class TfLiteVerifier {
 public:
  ...
  virtual ~TfLiteVerifier() {
   ...
   MHTracer_DTPStensorflowPSlitePScorePSapiPSverifierDTh mht_0(mht_0_v, 202, "", "./tensorflow/lite/core/api/verifier.h", "~TfLiteVerifier");
}
};

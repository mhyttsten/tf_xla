...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSutilDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSutilDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSutilDTh() {
      // Check if tracing is enabled
...
inline bool IsResourceOrVariant(const TfLiteTensor* tensor) {
   ...
   MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSutilDTh mht_0(mht_0_v, 220, "", "./tensorflow/lite/delegates/flex/util.h", "IsResourceOrVariant");

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSdelegate_symbolDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSdelegate_symbolDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSdelegate_symbolDTcc() {
      // Check if tracing is enabled
...
TFL_CAPI_EXPORT tflite::TfLiteDelegateUniquePtr TF_AcquireFlexDelegate() {
   ...
   MHTracer_DTPStensorflowPSlitePSdelegatesPSflexPSdelegate_symbolDTcc mht_0(mht_0_v, 206, "", "./tensorflow/lite/delegates/flex/delegate_symbol.cc", "TF_AcquireFlexDelegate");

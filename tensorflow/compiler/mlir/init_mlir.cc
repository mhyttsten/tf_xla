...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSmlirPSinit_mlirDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSmlirPSinit_mlirDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSmlirPSinit_mlirDTcc() {
      // Check if tracing is enabled
...
InitMlir::InitMlir(int *argc, char ***argv) : init_llvm_(*argc, *argv) {
   ...
   MHTracer_DTPStensorflowPScompilerPSmlirPSinit_mlirDTcc mht_0(mht_0_v, 202, "", "./tensorflow/compiler/mlir/init_mlir.cc", "InitMlir::InitMlir");

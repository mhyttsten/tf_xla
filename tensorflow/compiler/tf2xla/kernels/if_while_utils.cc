...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPStf2xlaPSkernelsPSif_while_utilsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPStf2xlaPSkernelsPSif_while_utilsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPStf2xlaPSkernelsPSif_while_utilsDTcc() {
      // Check if tracing is enabled
...
Status FindMustBeConstNodes(XlaOpKernelContext* ctx,
   ...
   MHTracer_DTPStensorflowPScompilerPStf2xlaPSkernelsPSif_while_utilsDTcc mht_0(mht_0_v, 250, "", "./tensorflow/compiler/tf2xla/kernels/if_while_utils.cc", "FindMustBeConstNodes");

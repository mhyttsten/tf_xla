...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh() {
      // Check if tracing is enabled
...
class XlaDeviceContext : public DeviceContext {
 public:
  ...
  xla::LocalClient* client() const {
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_0(mht_0_v, 244, "", "./tensorflow/compiler/jit/xla_device_context.h", "client");
 return client_; }
  se::Stream* stream() const override {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_1(mht_1_v, 248, "", "./tensorflow/compiler/jit/xla_device_context.h", "stream");
 return stream_.get(); }
  se::Stream* host_to_device_stream() const {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_2(mht_2_v, 252, "", "./tensorflow/compiler/jit/xla_device_context.h", "host_to_device_stream");

    return host_to_device_stream_.get();
  }
  se::Stream* device_to_device_stream(int index) const {
   std::vector<std::string> mht_3_v;
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_3(mht_3_v, 258, "", "./tensorflow/compiler/jit/xla_device_context.h", "device_to_device_stream");

    return device_to_device_streams_.at(index).get();
  }
  xla::TransferManager* transfer_manager() const {
   std::vector<std::string> mht_4_v;
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_4(mht_4_v, 264, "", "./tensorflow/compiler/jit/xla_device_context.h", "transfer_manager");
 return transfer_manager_; }
  const XlaShapeLayoutHelpers::ShapeDeterminationFns& shape_determination_fns()
      const {
   std::vector<std::string> mht_5_v;
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_5(mht_5_v, 269, "", "./tensorflow/compiler/jit/xla_device_context.h", "shape_determination_fns");
 ...
 private:
  bool UseMultipleStreams() const {
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPSxla_device_contextDTh mht_6(mht_6_v, 283, "", "./tensorflow/compiler/jit/xla_device_context.h", "UseMultipleStreams");

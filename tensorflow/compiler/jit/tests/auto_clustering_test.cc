...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSjitPStestsPSauto_clustering_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPStestsPSauto_clustering_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSjitPStestsPSauto_clustering_testDTcc() {
      // Check if tracing is enabled
...
class AutoClusteringTestImpl : public AutoClusteringTest {
 protected:
  ...
  Status RunAutoClusteringTestWithPbtxt(absl::string_view key) {
   ...
   mht_0_v.push_back("key: \"" + std::string(key.data(), key.size()) + "\"");
   MHTracer_DTPStensorflowPScompilerPSjitPStestsPSauto_clustering_testDTcc mht_0(mht_0_v, 195, "", "./tensorflow/compiler/jit/tests/auto_clustering_test.cc", "RunAutoClusteringTestWithPbtxt");
  ...
  Status RunAutoClusteringTestWithGzippedPbtxt(absl::string_view key) {
   ...
   mht_1_v.push_back("key: \"" + std::string(key.data(), key.size()) + "\"");
   MHTracer_DTPStensorflowPScompilerPSjitPStestsPSauto_clustering_testDTcc mht_1(mht_1_v, 209, "", "./tensorflow/compiler/jit/tests/auto_clustering_test.cc", "RunAutoClusteringTestWithGzippedPbtxt");
...
void BM_MarkForCompilationPass_KerasImagenetMain(benchmark::State& state) {
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPStestsPSauto_clustering_testDTcc mht_2(mht_2_v, 264, "", "./tensorflow/compiler/jit/tests/auto_clustering_test.cc", "BM_MarkForCompilationPass_KerasImagenetMain");

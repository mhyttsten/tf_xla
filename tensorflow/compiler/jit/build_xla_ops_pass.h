...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSjitPSbuild_xla_ops_passDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPSbuild_xla_ops_passDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSjitPSbuild_xla_ops_passDTh() {
      // Check if tracing is enabled
...
class BuildXlaOpsPass : public GraphOptimizationPass {
 public:
  ...
  explicit BuildXlaOpsPass(
   ...
   MHTracer_DTPStensorflowPScompilerPSjitPSbuild_xla_ops_passDTh mht_0(mht_0_v, 204, "", "./tensorflow/compiler/jit/build_xla_ops_pass.h", "BuildXlaOpsPass");
}

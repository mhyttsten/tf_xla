...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSiterator_utilDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSiterator_utilDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSiterator_utilDTh() {
      // Check if tracing is enabled
...
class UnwrappingIterator
 ...
 public:
  explicit UnwrappingIterator(NestedIter iter) : iter_(std::move(iter)) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSiterator_utilDTh mht_0(mht_0_v, 235, "", "./tensorflow/compiler/xla/iterator_util.h", "UnwrappingIterator");
}

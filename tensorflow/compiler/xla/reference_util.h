...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSreference_utilDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSreference_utilDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSreference_utilDTh() {
      // Check if tracing is enabled
...
class ReferenceUtil {
 ...
 private:
  ...
  static void AssertSameSize2D(const Array1& array1) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSreference_utilDTh mht_0(mht_0_v, 850, "", "./tensorflow/compiler/xla/reference_util.h", "AssertSameSize2D");
}

...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSlayout_util_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSlayout_util_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSlayout_util_testDTcc() {
      // Check if tracing is enabled
...
class LayoutUtilTest : public ::testing::Test {
 protected:
  Shape MakeShapeWithLayout(PrimitiveType element_type,
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSlayout_util_testDTcc mht_0(mht_0_v, 200, "", "./tensorflow/compiler/xla/layout_util_test.cc", "MakeShapeWithLayout");

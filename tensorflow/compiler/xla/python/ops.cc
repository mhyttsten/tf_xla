...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSopsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSopsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSopsDTcc() {
      // Check if tracing is enabled
...
void BuildOpsSubmodule(py::module* m) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSopsDTcc mht_0(mht_0_v, 212, "", "./tensorflow/compiler/xla/python/ops.cc", "BuildOpsSubmodule");

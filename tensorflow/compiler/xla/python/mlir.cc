...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSmlirDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSmlirDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSmlirDTcc() {
      // Check if tracing is enabled
...
void BuildMlirSubmodule(py::module& m) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSmlirDTcc mht_0(mht_0_v, 253, "", "./tensorflow/compiler/xla/python/mlir.cc", "BuildMlirSubmodule");

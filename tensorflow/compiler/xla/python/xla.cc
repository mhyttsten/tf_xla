...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSxlaDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSxlaDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSxlaDTcc() {
      // Check if tracing is enabled
...
bool IsOptimizedBuild() {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpythonPSxlaDTcc mht_0(mht_0_v, 240, "", "./tensorflow/compiler/xla/python/xla.cc", "IsOptimizedBuild");

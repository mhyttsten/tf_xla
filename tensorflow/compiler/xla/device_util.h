...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSdevice_utilDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSdevice_utilDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSdevice_utilDTh() {
      // Check if tracing is enabled
...
std::string DeviceIdentifier(se::StreamExecutor* stream_exec) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSdevice_utilDTh mht_0(mht_0_v, 201, "", "./tensorflow/compiler/xla/device_util.h", "DeviceIdentifier");

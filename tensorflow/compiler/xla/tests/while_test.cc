...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPStestsPSwhile_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPStestsPSwhile_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPStestsPSwhile_testDTcc() {
      // Check if tracing is enabled
...
XLA_TEST_F(WhileTest, WhileWithPrngScalarResult) {
  ...
  auto build_condition = [this, v6s32](int count) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPStestsPSwhile_testDTcc mht_0(mht_0_v, 1077, "", "./tensorflow/compiler/xla/tests/while_test.cc", "lambda");
  ...
  auto while_loop = [this, &body, build_condition](int count) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPStestsPSwhile_testDTcc mht_1(mht_1_v, 1104, "", "./tensorflow/compiler/xla/tests/while_test.cc", "lambda");
...
void BM_WhileLoop(::testing::benchmark::State& state) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPStestsPSwhile_testDTcc mht_2(mht_2_v, 1438, "", "./tensorflow/compiler/xla/tests/while_test.cc", "BM_WhileLoop");

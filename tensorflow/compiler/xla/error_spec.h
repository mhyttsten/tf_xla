...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSerror_specDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSerror_specDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSerror_specDTh() {
      // Check if tracing is enabled
...
struct ErrorSpec {
  explicit ErrorSpec(float aabs, float arel = 0, bool relaxed_nans = false)
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSerror_specDTh mht_0(mht_0_v, 193, "", "./tensorflow/compiler/xla/error_spec.h", "ErrorSpec");
}

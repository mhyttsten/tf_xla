...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSpjrtPSmetricsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpjrtPSmetricsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSpjrtPSmetricsDTcc() {
      // Check if tracing is enabled
...
void ReportExecutableEnqueueTime(const uint64_t running_time_usecs) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSpjrtPSmetricsDTcc mht_0(mht_0_v, 205, "", "./tensorflow/compiler/xla/pjrt/metrics.cc", "ReportExecutableEnqueueTime");

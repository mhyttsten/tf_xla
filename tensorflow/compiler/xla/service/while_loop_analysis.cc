...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSwhile_loop_analysisDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSwhile_loop_analysisDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSwhile_loop_analysisDTcc() {
      // Check if tracing is enabled
...
static const HloInstruction* NonConstantOperand(const HloInstruction* instr) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSwhile_loop_analysisDTcc mht_0(mht_0_v, 204, "", "./tensorflow/compiler/xla/service/while_loop_analysis.cc", "NonConstantOperand");
...
std::vector<const HloInstruction*> GetAuxiliaryLoopInductionVars(
  ...
  const auto add_dependencies = [](const HloInstruction* hlo,
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSwhile_loop_analysisDTcc mht_1(mht_1_v, 365, "", "./tensorflow/compiler/xla/service/while_loop_analysis.cc", "lambda");
...
static HloInstruction* GetOnlyGTE(HloInstruction* inst) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSwhile_loop_analysisDTcc mht_2(mht_2_v, 734, "", "./tensorflow/compiler/xla/service/while_loop_analysis.cc", "GetOnlyGTE");

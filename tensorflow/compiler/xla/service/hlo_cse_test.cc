...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSservicePShlo_cse_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePShlo_cse_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSservicePShlo_cse_testDTcc() {
      // Check if tracing is enabled
...
class HloCseTest : public HloTestBase {
 protected:
  HloCseTest() {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePShlo_cse_testDTcc mht_0(mht_0_v, 216, "", "./tensorflow/compiler/xla/service/hlo_cse_test.cc", "HloCseTest");
}
};

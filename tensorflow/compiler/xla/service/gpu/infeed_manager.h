...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSinfeed_managerDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSinfeed_managerDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSinfeed_managerDTh() {
      // Check if tracing is enabled
...
class InfeedManager
 ...
 private:
  se::Stream* stream() const {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSinfeed_managerDTh mht_0(mht_0_v, 224, "", "./tensorflow/compiler/xla/service/gpu/infeed_manager.h", "stream");
 return stream_.get(); }

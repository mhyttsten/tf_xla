...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPStriangular_solve_rewriterDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPStriangular_solve_rewriterDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPStriangular_solve_rewriterDTcc() {
      // Check if tracing is enabled
...
StatusOr<bool> TriangularSolveRewriter::Run(HloModule* module) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPStriangular_solve_rewriterDTcc mht_0(mht_0_v, 195, "", "./tensorflow/compiler/xla/service/gpu/triangular_solve_rewriter.cc", "TriangularSolveRewriter::Run");
  ...
  for (HloComputation* comp : module->MakeNonfusionComputations()) {
    ...
    for (HloInstruction* instr : to_rewrite) {
      ...
      int64_t batch_size = std::accumulate(
          ...
          int64_t{1}, [](int64_t a, int64_t b) {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPStriangular_solve_rewriterDTcc mht_1(mht_1_v, 212, "", "./tensorflow/compiler/xla/service/gpu/triangular_solve_rewriter.cc", "lambda");
 return a * b; });

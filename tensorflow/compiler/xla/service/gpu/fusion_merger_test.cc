...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSfusion_merger_testDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSfusion_merger_testDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSfusion_merger_testDTcc() {
      // Check if tracing is enabled
...
TEST_F(FusionMergerTest, AvoidsLargeFusion) {
  ...
  auto make_fusion = [&](absl::Span<HloInstruction* const> params) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSgpuPSfusion_merger_testDTcc mht_0(mht_0_v, 504, "", "./tensorflow/compiler/xla/service/gpu/fusion_merger_test.cc", "lambda");

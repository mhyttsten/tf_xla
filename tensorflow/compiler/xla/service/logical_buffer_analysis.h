...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSlogical_buffer_analysisDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSlogical_buffer_analysisDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSlogical_buffer_analysisDTh() {
      // Check if tracing is enabled
...
class LogicalBufferAnalysis : public DfsHloVisitorWithDefault {
 public:
  ...
  const std::vector<std::unique_ptr<LogicalBuffer>>& logical_buffers() const {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSlogical_buffer_analysisDTh mht_0(mht_0_v, 211, "", "./tensorflow/compiler/xla/service/logical_buffer_analysis.h", "logical_buffers");

    return logical_buffers_;
  }
  size_t num_logical_buffers() const {
   std::vector<std::string> mht_1_v;
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSlogical_buffer_analysisDTh mht_1(mht_1_v, 217, "", "./tensorflow/compiler/xla/service/logical_buffer_analysis.h", "num_logical_buffers");
 return logical_buffers_.size(); }

 private:
  explicit LogicalBufferAnalysis(const HloModule* module) : module_(module) {
   std::vector<std::string> mht_2_v;
   MHTracer_DTPStensorflowPScompilerPSxlaPSservicePSlogical_buffer_analysisDTh mht_2(mht_2_v, 223, "", "./tensorflow/compiler/xla/service/logical_buffer_analysis.h", "LogicalBufferAnalysis");
}
  Status Analyze();

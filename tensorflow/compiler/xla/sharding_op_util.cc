...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSsharding_op_utilDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSsharding_op_utilDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSsharding_op_utilDTcc() {
      // Check if tracing is enabled
...
std::string EncodeAttributes(absl::Span<const int64_t> unspecified_dims) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSsharding_op_utilDTcc mht_0(mht_0_v, 200, "", "./tensorflow/compiler/xla/sharding_op_util.cc", "EncodeAttributes");
...
Status ParseAttributes(absl::string_view opaque,
   ...
   mht_1_v.push_back("opaque: \"" + std::string(opaque.data(), opaque.size()) + "\"");
   MHTracer_DTPStensorflowPScompilerPSxlaPSsharding_op_utilDTcc mht_1(mht_1_v, 213, "", "./tensorflow/compiler/xla/sharding_op_util.cc", "ParseAttributes");

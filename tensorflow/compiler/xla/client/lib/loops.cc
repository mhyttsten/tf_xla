...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxlaPSclientPSlibPSloopsDTcc {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSclientPSlibPSloopsDTcc(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxlaPSclientPSlibPSloopsDTcc() {
      // Check if tracing is enabled
...
StatusOr<std::vector<XlaOp>> WhileLoopHelper(
  ...
  auto unpack_tuple = [](XlaOp tuple, int arity, XlaBuilder* builder) {
   ...
   MHTracer_DTPStensorflowPScompilerPSxlaPSclientPSlibPSloopsDTcc mht_0(mht_0_v, 209, "", "./tensorflow/compiler/xla/client/lib/loops.cc", "lambda");

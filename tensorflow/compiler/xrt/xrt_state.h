...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSxrtPSxrt_stateDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSxrtPSxrt_stateDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSxrtPSxrt_stateDTh() {
      // Check if tracing is enabled
...
class XRTBufferAllocation : public core::RefCounted {
 public:
  ...
  void DiscardAllocation() {
   ...
   MHTracer_DTPStensorflowPScompilerPSxrtPSxrt_stateDTh mht_0(mht_0_v, 229, "", "./tensorflow/compiler/xrt/xrt_state.h", "DiscardAllocation");

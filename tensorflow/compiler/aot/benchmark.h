...
#include <stdlib.h>
#include <unistd.h>
class MHTracer_DTPStensorflowPScompilerPSaotPSbenchmarkDTh {
public:
   std::string _s;
   int _indent = 0;
   ...
   MHTracer_DTPStensorflowPScompilerPSaotPSbenchmarkDTh(std::vector<std::string> params, int lineNumber, std::string prefix, std::string fileName, std::string functionName) {
   ...
   ~MHTracer_DTPStensorflowPScompilerPSaotPSbenchmarkDTh() {
      // Check if tracing is enabled
...
struct Stats {
  ...
  Stats() : total_us(0) {
   ...
   MHTracer_DTPStensorflowPScompilerPSaotPSbenchmarkDTh mht_0(mht_0_v, 221, "", "./tensorflow/compiler/aot/benchmark.h", "Stats");
